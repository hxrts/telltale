...
Solution Structure. Prove `store_typed_rewrite_g` for GEnv substitution,
`store_typed_rewrite_s` for SEnv substitution, and monotonicity variants.
...
lemma store_typed_rewrite_g {G G' : GEnv} {S : SEnv} {store : VarStore}
    (hMono : ∀ e L, lookupG G e = some L → lookupG G' e = some L) :
    StoreTyped G S store → StoreTyped G' S store := by
  intro hStore x v T hStr hS
  exact has_type_val_mono G G' v T (hStore x v T hStr hS) hMono

lemma store_typed_rewrite_s {G : GEnv} {S S' : SEnv} {store : VarStore}
...
lemma store_typed_weaken_s {G : GEnv} {S S' : SEnv} {store : VarStore}
    (hMono : ∀ x T, lookupSEnv S' x = some T → lookupSEnv S x = some T) :
    StoreTyped G S store → StoreTyped G S' store := by
  intro hStore x v T hStr hS'
  exact hStore x v T hStr (hMono x T hS')

lemma store_typed_strong_rewrite_g {G G' : GEnv} {S : SEnv} {store : VarStore}
  ...
  ·
    apply store_typed_rewrite_g (G:=G) (G':=G') (S:=S)
      (hMono:=by
        intro e L hLookup
        simpa [hEq e] using hLookup)
    exact hStore.typeCorr

lemma store_typed_strong_rewrite_s {G : GEnv} {S S' : SEnv} {store : VarStore}
    (hEq : ∀ x, lookupSEnv S x = lookupSEnv S' x) :
    StoreTypedStrong G S store → StoreTypedStrong G S' store := by
  intro hStore
  refine ⟨?_, ?_⟩
  ...
  ·
    apply store_typed_rewrite_s (G:=G) (S:=S) (S':=S') hEq
    exact hStore.typeCorr

-- Disjoint-Lookup Commutation

lemma lookup_g_none_of_disjoint_early {G₁ G₂ : GEnv} (hDisj : DisjointG G₁ G₂)
...
lemma lookup_g_comm_of_disjoint_early {G₁ G₂ : GEnv} (hDisj : DisjointG G₁ G₂) :
    ∀ e, lookupG (G₁ ++ G₂) e = lookupG (G₂ ++ G₁) e := by
  intro e
  cases hLeft : lookupG G₁ e with
  | some L =>
      have hNone : lookupG G₂ e = none :=
        lookup_g_none_of_disjoint_early (G₁:=G₂) (G₂:=G₁) (disjoint_g_symm hDisj) (e:=e) (L:=L) hLeft
...
/- ## Structured Block 3 -/
      have hNone : lookupG (G₂ ++ G₁) e = none := by
        simpa [hSwap] using hLeft
      have hB := lookup_g_append_right (G₁:=G₂ ++ G₁) (G₂:=G₃) (e:=e) hNone
...
lemma store_typed_strong_swap_g_left {G₁ G₂ G₃ : GEnv} {S : SEnv} {store : VarStore}
    (hDisj : DisjointG G₁ G₂) :
    StoreTypedStrong ((G₁ ++ G₂) ++ G₃) S store →
    StoreTypedStrong ((G₂ ++ G₁) ++ G₃) S store := by
  intro hStore
  apply store_typed_strong_rewrite_g (G:=((G₁ ++ G₂) ++ G₃)) (G':=((G₂ ++ G₁) ++ G₃))
    (by
      intro e
      exact lookup_g_swap_left (G₁:=G₁) (G₂:=G₂) (G₃:=G₃) hDisj e)
  exact hStore

lemma store_typed_strong_swap_s_left_prefix
  ...
  intro hStore
  apply store_typed_strong_rewrite_s
    (S:=SEnvAll Ssh ((S₁ ++ S₂) ++ S₃))
    (S':=SEnvAll Ssh (S₂ ++ (S₁ ++ S₃)))
    (by
      intro x
      exact lookup_s_env_swap_left_prefix (Ssh:=Ssh) (S₁:=S₁) (S₂:=S₂) (S₃:=S₃) hDisj x)
  exact hStore

-- Store-Domain Update Lemmas

theorem lookup_s_env_none_of_disjoint_update
  ...
  | some T₂ =>
      have hS1 : lookupSEnv (updateSEnv S₁ x T) x = some T := by
        simpa using (lookup_s_env_update_eq (env:=S₁) (x:=x) (T:=T))
      exact (hDisj x T T₂ hS1 hS2).elim

/-- Same-domain is preserved by a matching SEnv/store update. -/
theorem store_typed_strong_same_domain_update
  ...
  · subst hEq
/- ## Structured Block 4 -/
    simp [lookup_s_env_update_eq, lookup_str_update_eq]
  · have hS : lookupSEnv (updateSEnv S x T) y = lookupSEnv S y := by
      simpa using (lookup_s_env_update_neq (env:=S) (x:=x) (y:=y) (T:=T) (Ne.symm hEq))
    have hStr : lookupStr (updateStr store x v) y = lookupStr store y := by
      simpa using (lookup_str_update_neq store x y v (Ne.symm hEq))
...
theorem store_typed_strong_update_g
  ...
  ·
    have hST : StoreTyped (updateG G e L) S store :=
      store_typed_send_preserved (G:=G) (S:=S) (store:=store) (e:=e) (L:=L) hStore.typeCorr
    exact hST

/-- Assignment preserves StoreTypedStrong on shared+owned SEnv. -/
theorem store_typed_strong_assign_update
  ...
  ·
    ...
    simpa [SEnvAll, update_s_env_append_left hNone] using hDom
  ·
    have hST : StoreTyped G (updateSEnv (SEnvAll Ssh Sown) x T) (updateStr store x v) :=
      store_typed_assign_preserved (G:=G) (S:=SEnvAll Ssh Sown) (store:=store) (x:=x) (v:=v) (T:=T)
        hStore.typeCorr hv
    simpa [SEnvAll, update_s_env_append_left hNone] using hST

/-- Receive preserves StoreTypedStrong on shared+owned SEnv. -/
theorem store_typed_strong_recv_update
  ...
  ·
    have hST : StoreTyped (updateG G e L) (updateSEnv (SEnvAll Ssh Sown) x T) (updateStr store x v) :=
      store_typed_recv_preserved (G:=G) (S:=SEnvAll Ssh Sown) (store:=store) (e:=e) (L:=L)
...
/- ## Structured Block 5 -/
lemma lookup_s_env_append_erase_ne
  ...
  have hEraseNe : lookupSEnv (eraseSEnv Sright x) y = lookupSEnv Sright y :=
    lookup_s_env_erase_ne (S:=Sright) (x:=x) (y:=y) hxy
  cases hR : lookupSEnv Sright y with
  | some Ty =>
      have hEraseSome : lookupSEnv (eraseSEnv Sright x) y = some Ty := by
        simpa [hR] using hEraseNe
      have hL :=
        lookup_s_env_append_left (S₁:=eraseSEnv Sright x) (S₂:=Srest) (x:=y) (T:=Ty) hEraseSome
      have hRhs :=
        lookup_s_env_append_left (S₁:=Sright) (S₂:=Srest) (x:=y) (T:=Ty) hR
      simpa [hL, hRhs]
  | none =>
      have hEraseNone : lookupSEnv (eraseSEnv Sright x) y = none := by
        simpa [hR] using hEraseNe
      have hL :=
        lookup_s_env_append_right (S₁:=eraseSEnv Sright x) (S₂:=Srest) (x:=y) hEraseNone
      have hRhs :=
        lookup_s_env_append_right (S₁:=Sright) (S₂:=Srest) (x:=y) hR
      simpa [hRhs] using hL

theorem lookup_s_env_update_left_frame_eq_update_s_env
  ...
  · subst y
    ...
    have hPrefixNone : lookupSEnv (Ssh ++ eraseSEnv Sown.right x) x = none := by
      have hAppend := lookup_s_env_append_right (S₁:=Ssh) (S₂:=eraseSEnv Sown.right x) (x:=x) hNoSh
      simpa [lookup_s_env_erase_eq (S:=Sown.right) (x:=x)] using hAppend
    ...
    have hUpdate : lookupSEnv (updateSEnv (SEnvAll Ssh (Sown ++ S₂)) x T) x = some T := by
      simpa using (lookup_s_env_update_eq (env:=SEnvAll Ssh (Sown ++ S₂)) (x:=x) (T:=T))
...
/- ## Structured Block 6 -/
  · have hTargetBase :
      ...
      have hInner :
        ...
        lookup_s_env_s_env_all_update_neq (Ssh:=Ssh ++ eraseSEnv Sown.right x) (Sown:=Sown.left) (S₂:=S₂)
          (x:=x) (y:=y) (T:=T) hxy
      have hEraseMid :
          lookupSEnv (SEnvAll (Ssh ++ eraseSEnv Sown.right x) (Sown.left ++ S₂)) y =
        ...
        | some Ty =>
            have hL :
              ...
              lookup_s_env_append_left (S₁:=Ssh) (S₂:=eraseSEnv Sown.right x ++ (Sown.left ++ S₂))
                (x:=y) (T:=Ty) hSh
            have hR :
                lookupSEnv (Ssh ++ (Sown.right ++ (Sown.left ++ S₂))) y = some Ty :=
              lookup_s_env_append_left (S₁:=Ssh) (S₂:=Sown.right ++ (Sown.left ++ S₂))
                (x:=y) (T:=Ty) hSh
        ...
        | none =>
            have hSuffix :
              ...
              lookup_s_env_append_erase_ne (Sright:=Sown.right) (Srest:=Sown.left ++ S₂) (x:=x) (y:=y) hxy
            have hL :
                lookupSEnv (Ssh ++ (eraseSEnv Sown.right x ++ (Sown.left ++ S₂))) y =
                  lookupSEnv (eraseSEnv Sown.right x ++ (Sown.left ++ S₂)) y :=
              lookup_s_env_append_right (S₁:=Ssh) (S₂:=eraseSEnv Sown.right x ++ (Sown.left ++ S₂))
                (x:=y) hSh
            have hR :
                lookupSEnv (Ssh ++ (Sown.right ++ (Sown.left ++ S₂))) y =
                  lookupSEnv (Sown.right ++ (Sown.left ++ S₂)) y :=
              lookup_s_env_append_right (S₁:=Ssh) (S₂:=Sown.right ++ (Sown.left ++ S₂))
                (x:=y) hSh
    ...
    have hUpdateNe :
        lookupSEnv (updateSEnv (SEnvAll Ssh (Sown ++ S₂)) x T) y =
          lookupSEnv (SEnvAll Ssh (Sown ++ S₂)) y := by
      simpa using
        (lookup_s_env_update_neq (env:=SEnvAll Ssh (Sown ++ S₂)) (x:=x) (y:=y) (T:=T)
          (Ne.symm hxy))
...
theorem store_typed_strong_frame_send
...
theorem store_typed_strong_frame_select
    {G G₂ : GEnv} {Ssh Sown S₂ : SEnv} {store : VarStore}
...
theorem store_typed_strong_frame_branch
    {G G₂ : GEnv} {Ssh Sown S₂ : SEnv} {store : VarStore}
