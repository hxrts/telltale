...
Bridge theorem from typed operational steps (`TypedStep`) to raw runtime
steps (`Step`) for the same concrete configuration transition.
-/

/- 
The Problem. We need a soundness bridge from the proof-carrying typed step
judgment to the executable step relation used by runtime semantics.

Solution Structure. Prove `subject_reduction` by induction on `TypedStep`,
