...
theorem isFreeIn_subst_other (e : LocalTypeR) (v t : String) (repl : LocalTypeR)
  ...
  | .var w =>
      ...
      simp only [isFreeIn] at hfree
      by_cases hwt : w == t
      · -- w == t means w = t, but v = w (from hfree) and v ≠ t, contradiction
        simp only [beq_iff_eq] at hfree hwt
        rw [hfree] at hvt; exact absurd hwt hvt
      · simp only [hwt, Bool.false_eq_true, ↓reduceIte]; exact hfree
  | .send p bs =>
      simp only [substitute_send, isFreeIn]
      simp only [isFreeIn] at hfree
      exact isFreeInBranches_subst_other bs v t repl hvt hfree
  | .recv p bs =>
      simp only [substitute_recv, isFreeIn]
      simp only [isFreeIn] at hfree
      exact isFreeInBranches_subst_other bs v t repl hvt hfree
  | .mu s body =>
      unfold LocalTypeR.substitute
      by_cases hst : s == t
      · simp only [hst, ↓reduceIte]; exact hfree
      · simp only [hst, Bool.false_eq_true, ↓reduceIte, isFreeIn]
        simp only [isFreeIn] at hfree
        by_cases hvs : v == s
        · -- v == s means v = s which means v is bound, so isFreeIn v (.mu s body) = false
          -- But hfree says it's true. After simp, this simplifies hfree to false = true.
          simp only [hvs, ↓reduceIte] at hfree
          cases hfree  -- hfree : false = true is absurd
...
/-- Substituting `.end` for an unguarded variable produces something EQ2 to `.end`.
    ...
    This is the main theorem that eliminates the axiom in Harmony.lean. -/
