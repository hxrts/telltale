...
/-! # SessionTypes.GlobalType.ProductivityLemmas

isProductive preservation under substitution and well_formed_mu_unfold theorem.
-/
...
Solution Structure. Proves `is_productive_cons_dedup` and `is_productive_swap` for
manipulating the unguarded list. Shows productivity is preserved under substitution
via mutual recursion. The main theorem `well_formed_mu_unfold` combines all preservation
...
theorem is_productive_cons_dedup (g : GlobalType) (x : String) (unguarded : List String)
    (hprod : g.isProductive (x :: x :: unguarded) = true) :
    g.isProductive (x :: unguarded) = true := by
  apply is_productive_mono g (x :: unguarded) (x :: x :: unguarded)
  · intro y hy
...
theorem is_productive_swap (g : GlobalType) (x y : String) (unguarded : List String)
  ...
  apply is_productive_mono g (y :: x :: unguarded) (x :: y :: unguarded)
  · intro z hz
...
mutual
  ...
  theorem is_productive_extend (g : GlobalType) (bound extra : List String)
    ...
    | .mu t body =>
        ...
        rw [h]
        exact is_productive_extend body (t :: bound) extra hbound hprod
    | .comm _ _ branches =>
        simp only [GlobalType.allVarsBound] at hbound
        simp only [GlobalType.isProductive] at hprod ⊢
  ...
  theorem is_productive_branches_extend (branches : List (Label × GlobalType))
    ...
    | (_, g) :: rest =>
        ...
        exact ⟨is_productive_extend g bound extra hbound.1 hprod.1,
               is_productive_branches_extend rest bound extra hbound.2 hprod.2⟩
end

/-- Corollary: If (mu t body) is productive, then it's productive for any unguarded list. -/
theorem mu_is_productive_forall (t : String) (body : GlobalType)
...
mutual
  private theorem is_productive_substitute_var (t varName : String) (repl : GlobalType)
  ...
  private theorem is_productive_substitute_mu_shadow (t varName : String) (inner : GlobalType)
      (unguarded : List String)
    ...
    subst heq'
    exact is_productive_cons_dedup inner t unguarded hg
  ...
  theorem is_productive_substitute (g : GlobalType) (varName : String) (repl : GlobalType)
      (unguarded : List String)
    ...
    | .var t => exact is_productive_substitute_var t varName repl unguarded hg hrepl
    | .mu t inner =>
        simp only [GlobalType.substitute]
        split
        · -- t = varName, shadowed - no substitution
          ...
          exact is_productive_substitute_mu_shadow t varName inner unguarded hg heq
        · -- t ≠ varName
          simp only [GlobalType.isProductive] at hg ⊢
          have hg' : inner.isProductive (varName :: t :: unguarded) = true :=
            is_productive_swap inner t varName unguarded hg
          exact is_productive_substitute inner varName repl (t :: unguarded) hg' hrepl
    | .comm sender receiver branches =>
        ...
        exact is_productive_branches_substitute_any branches varName repl [] hg hrepl
/- ## Structured Block 1 -/
    | .delegate p q sid r cont =>
        simp only [GlobalType.substitute, GlobalType.isProductive] at hg ⊢
        -- delegate resets unguarded to [], so hg : cont.isProductive [] = true
        exact is_productive_substitute_any cont varName repl [] hg hrepl
  ...
  theorem is_productive_branches_substitute_any (branches : List (Label × GlobalType))
    ...
    | (label, cont) :: rest =>
        ...
        exact ⟨is_productive_substitute_any cont varName repl unguarded hg.1 hrepl,
               is_productive_branches_substitute_any rest varName repl unguarded hg.2 hrepl⟩
  ...
  theorem is_productive_substitute_any (g : GlobalType) (varName : String) (repl : GlobalType)
      (unguarded : List String)
    ...
    | .mu t inner =>
        ...
        · exact hg
        · simp only [GlobalType.isProductive] at hg ⊢
          exact is_productive_substitute_any inner varName repl (t :: unguarded) hg hrepl
    | .comm sender receiver branches =>
        simp only [GlobalType.substitute, GlobalType.isProductive] at hg ⊢
        exact is_productive_branches_substitute_any branches varName repl [] hg hrepl
    | .delegate p q sid r cont =>
...
and isProductive. -/
theorem well_formed_mu_unfold_partial (t : String) (body : GlobalType)
  ...
  exact ⟨no_self_comm_substitute body t (GlobalType.mu t body) hns hns,
         all_comms_non_empty_substitute body t (GlobalType.mu t body) hne hne⟩
...
theorem all_vars_bound_mu_unfold (t : String) (body : GlobalType)
  ...
  have hrepl : (GlobalType.mu t body).allVarsBound [] = true := by
    ...
    exact hbound
  exact all_vars_bound_substitute body t (GlobalType.mu t body) [] hbound hrepl
...
theorem is_productive_mu_unfold (t : String) (body : GlobalType)
    (hbound : (GlobalType.mu t body).allVarsBound [] = true)
...
theorem well_formed_mu_unfold (t : String) (body : GlobalType)
    (hwf : (GlobalType.mu t body).wellFormed = true) :
  ...
  constructor
  constructor
  · exact all_vars_bound_mu_unfold t body hbound
  · exact all_comms_non_empty_substitute body t (GlobalType.mu t body) hne hne
  · exact no_self_comm_substitute body t (GlobalType.mu t body) hns hns
  · exact is_productive_mu_unfold t body hbound hprod
