import Telltale.Proofs.Projection.Harmony.Part2
...
open Telltale.Protocol.Projection.Project (ProjectableClosedWellFormed)
open Telltale.Protocol.Projection.Projectb (CProject)

abbrev projTransBranches := Telltale.Protocol.Projection.Trans.transBranches
...
private theorem mem_transBranches_of_mem (branches : List (Label × GlobalType))
  ...
  | cons hd tl ih =>
      ...
      | mk label_hd cont_hd =>
        cases hmem with
        | head =>
            simp [projTransBranches, Telltale.Protocol.Projection.Trans.transBranches]
        | tail _ htl =>
            have ih' := ih htl
            simp [projTransBranches, Telltale.Protocol.Projection.Trans.transBranches, List.mem_cons, ih']
...
private theorem proj_trans_other_step_comm_async_other_cons
  ...
  have hbranch_rel' :
    ...
    simpa [projTransBranches, Telltale.Protocol.Projection.Trans.transBranches] using hbranch_rel
