import Telltale.Proofs.Projection.Harmony.Part1
...
namespace Telltale.Proofs.Projection.Harmony
...
open Telltale.Protocol.Projection.Trans (trans_comm_sender trans_comm_receiver trans_comm_other)
...
private abbrev gSubstBranches := Telltale.Protocol.GlobalType.substituteBranches -- global branch subst
private abbrev lSubstBranches := Telltale.Protocol.LocalTypeR.substituteBranches -- local branch subst
private abbrev projTransBranches := Telltale.Protocol.Projection.Trans.transBranches
...
private theorem transBranches_ProjSubstRel (t : String) (G : GlobalType) (role : String)
  ...
  | nil =>
      simp [BranchesRel, gSubstBranches, lSubstBranches, projTransBranches,
        Telltale.Protocol.Projection.Trans.transBranches,
        Telltale.Protocol.GlobalType.substituteBranches,
        Telltale.Protocol.LocalTypeR.substituteBranches,
        -Telltale.Protocol.LocalTypeR.substituteBranches_eq_map]
  | cons hd tl ih =>
      ...
      ·
        simpa [BranchesRel, gSubstBranches, lSubstBranches, projTransBranches,
          ...
          Telltale.Protocol.GlobalType.substituteBranches,
          Telltale.Protocol.LocalTypeR.substituteBranches,
          -Telltale.Protocol.LocalTypeR.substituteBranches_eq_map] using ih
...
private theorem ProjSubstRel_EQ2F_end (t : String) (G : GlobalType) (role : String) :
  ...
  simp [GlobalType.substitute, projTrans, Protocol.Projection.Trans.trans, EQ2F]
...
private theorem ProjSubstRel_EQ2F_var (v t : String) (G : GlobalType) (role : String) :
  ...
  · have hvt' : (v == t) = false := beq_eq_false_iff_ne.mpr hvt
    simp [GlobalType.substitute, projTrans, Protocol.Projection.Trans.trans, hvt']
    rfl
...
private theorem ProjSubstRel_EQ2F_mu_shadow
  ...
  ·
    have hguard_t : (Protocol.Projection.Trans.trans inner role).isGuarded t = true := by
      simpa [hst] using hguard
    have hF : EQ2F EQ2
        (LocalTypeR.mu s (Protocol.Projection.Trans.trans inner role))
        (LocalTypeR.mu s (Protocol.Projection.Trans.trans inner role)) :=
...
private theorem ProjSubstRel_EQ2F_mu_noshadow_guarded
    ...
    (hguardL : (Protocol.Projection.Trans.trans (inner.substitute t G) role).isGuarded s = true)
    (hguardR : (Protocol.Projection.Trans.trans inner role).isGuarded s = true) :
    EQ2F (fun x y => ProjSubstRel t G role x y ∨ EQ2 x y)
      (projTrans (GlobalType.mu s (inner.substitute t G)) role)
      ((projTrans (GlobalType.mu s inner) role).substitute t (projTrans G role)) := by
  -- Both sides are guarded mu; relate via crossed unfold lemmas.
  simp [projTrans, Protocol.Projection.Trans.trans, hguardL, hguardR]
...
private theorem ProjSubstRel_EQ2F_mu_noshadow_guarded_end
    ...
    (hsne : s ≠ t) (hGclosed : G.isClosed = true)
    (hguardL : (Protocol.Projection.Trans.trans (inner.substitute t G) role).isGuarded s = true)
    (hguardR : (Protocol.Projection.Trans.trans inner role).isGuarded s = false) :
    EQ2F (fun x y => ProjSubstRel t G role x y ∨ EQ2 x y)
      (projTrans (GlobalType.mu s (inner.substitute t G)) role)
      ((projTrans (GlobalType.mu s inner) role).substitute t (projTrans G role)) := by
...
private theorem ProjSubstRel_EQ2F_mu_noshadow_end_guarded
    (s t : String) (inner G : GlobalType) (role : String)
    (hsne : s ≠ t) (hGclosed : G.isClosed = true)
    (hguardL : (Protocol.Projection.Trans.trans (inner.substitute t G) role).isGuarded s = false)
    (hguardR : (Protocol.Projection.Trans.trans inner role).isGuarded s = true) :
    EQ2F (fun x y => ProjSubstRel t G role x y ∨ EQ2 x y)
      (projTrans (GlobalType.mu s (inner.substitute t G)) role)
      ((projTrans (GlobalType.mu s inner) role).substitute t (projTrans G role)) := by
...
private theorem ProjSubstRel_EQ2F_mu_noshadow_end_end
  ...
  simp [projTrans, Protocol.Projection.Trans.trans, hguardL, hguardR, EQ2F]
...
private theorem ProjSubstRel_EQ2F_mu_noshadow
  ...
  cases hguardL : (Protocol.Projection.Trans.trans (inner.substitute t G) role).isGuarded s with
  | true =>
      cases hguardR : (Protocol.Projection.Trans.trans inner role).isGuarded s with
      | true =>
...
private theorem ProjSubstRel_EQ2F_comm_sender
  ...
  simpa [lSubstBranches, Telltale.Protocol.LocalTypeR.substituteBranches_eq_map] using
...
private theorem ProjSubstRel_EQ2F_comm_other_nil
  ...
  have hLHS : projTrans (GlobalType.comm sender receiver (gSubstBranches [] t G)) role = LocalTypeR.end := by
    ...
    simp [gSubstBranches, Telltale.Protocol.GlobalType.substituteBranches] at h
    exact h
...
open Telltale.Proofs.Projection.SubstEndUnguarded (subst_end_unguarded_eq2_end)
...
private theorem trans_substitute_unfold_guarded
  ...
  have h_proj_mu : projTrans (.mu t body) role = LocalTypeR.mu t (projTrans body role) := by
    simp [projTrans, Protocol.Projection.Trans.trans, hguard]
...
private theorem trans_substitute_unfold_unguarded
  ...
  have hproj_subst :
    ...
    Telltale.Protocol.Projection.ProjSubst.proj_subst_mu_self t body role hclosed
  have hproj_subst' :
      projTrans (body.substitute t (.mu t body)) role =
...
end Telltale.Proofs.Projection.Harmony
