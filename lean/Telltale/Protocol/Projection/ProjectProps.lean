...
/-- Projection is deterministic up to EQ2 for well-formed globals. -/
def ProjectDeterministic (g : GlobalType) (role : String) (e1 e2 : LocalTypeR) : Prop :=
  CProject g role e1 → CProject g role e2 → g.wellFormed = true → EQ2 e1 e2
...
def BranchesProjDeterministic (gbs : List (Label × GlobalType)) (role : String)
  ...
  BranchesRel EQ2 lbs1 lbs2

/-- Claims bundle for projection determinism. -/
structure Claims where
  /-- Projection is deterministic up to EQ2. -/
  project_deterministic :
    ∀ g role e1 e2, ProjectDeterministic g role e1 e2
  /-- Branch-wise projection is deterministic up to EQ2. -/
  branches_proj_deterministic :
    ∀ gbs role lbs1 lbs2, BranchesProjDeterministic gbs role lbs1 lbs2
...
theorem project_deterministic {g : GlobalType} {role : String} {e1 e2 : LocalTypeR}
  ...
  -- Reduce both projections to trans and conclude by equality.
...
theorem branches_proj_deterministic {gbs : List (Label × GlobalType)} {role : String}
  ...
  -- Induct over the branch list and use the head lemma.
  cases gbs with
  | nil =>
      cases h1
      cases h2
...
/-- Assemble the determinism claims. -/
def claims : Claims := by
  -- Package the two determinism theorems.
  refine ⟨?_, ?_⟩
  · intro g role e1 e2
    exact project_deterministic
  · intro gbs role lbs1 lbs2
    exact branches_proj_deterministic
