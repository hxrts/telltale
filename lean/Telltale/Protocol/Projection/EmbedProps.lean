...
/-! # Telltale.Protocol.Projection.EmbedProps
...
private lemma embed_lcontractive_mu_case {t : String} {body' : LocalTypeR}
    ...
    Telltale.Protocol.Projection.Trans.lcontractive (GlobalType.mu t' gbody') = true := by
  cases body' with
  | «end» =>
      cases gbody' with
      | «end» => simp [Telltale.Protocol.Projection.Trans.lcontractive]
  ...
  | send _ _ =>
      ...
      | var _ => simp [CEmbedF] at hinner'
      | mu _ _ => simp [CEmbedF] at hinner'
      | comm _ _ _ => simp [Telltale.Protocol.Projection.Trans.lcontractive]
  | recv _ _ =>
      cases gbody' with
      | «end» => simp [CEmbedF] at hinner'
      | var _ => simp [CEmbedF] at hinner'
...
private lemma embed_lcontractive_of_local {body : LocalTypeR} {role : String} {gbody : GlobalType}
    ...
    Telltale.Protocol.Projection.Trans.lcontractive gbody = true := by
  ...
  | «end» | var _ | send _ _ | recv _ _ =>
      cases gbody <;>
        simp [CEmbedF, LocalTypeR.lcontractive,
          Telltale.Protocol.Projection.Trans.lcontractive] at hF hcontr ⊢
