...
/-! # Telltale.Protocol.Projection.Embed
...
namespace Telltale.Protocol.Projection.Embed
...
open Telltale.Protocol.Projection.Trans (lcontractive)
...
theorem CEmbed_gfp : CEmbed = Telltale.Protocol.CoTypes.CoinductiveRel.gfp (F := CEmbedF) := rfl

/-- Coinduction principle for CEmbed. -/
theorem CEmbed_coind' {R : EmbedRel} (h : R ≤ CEmbedF R) : R ≤ CEmbed := by
  exact Telltale.Protocol.CoTypes.CoinductiveRel.coind (F := CEmbedF) h
...
theorem CEmbed_unfold' : CEmbed ≤ CEmbedF CEmbed := by
  ...
  exact Telltale.Protocol.CoTypes.CoinductiveRel.unfold (F := CEmbedF)
...
theorem CEmbed_fold' : CEmbedF CEmbed ≤ CEmbed := by
  change CEmbedF (OrderHom.gfp ⟨CEmbedF, CEmbedF_mono⟩) ≤
    OrderHom.gfp ⟨CEmbedF, CEmbedF_mono⟩
  exact Telltale.Protocol.CoTypes.CoinductiveRel.fold (F := CEmbedF)

/-- CEmbed is a fixed point. -/
private theorem CEmbed_fixed : CEmbedF CEmbed = CEmbed := by
  change CEmbedF (OrderHom.gfp ⟨CEmbedF, CEmbedF_mono⟩) =
    OrderHom.gfp ⟨CEmbedF, CEmbedF_mono⟩
  exact Telltale.Protocol.CoTypes.CoinductiveRel.gfp_fixed (F := CEmbedF)
...
end Telltale.Protocol.Projection.Embed
