...
def Bisim_gfp : Rel := Telltale.Protocol.CoTypes.CoinductiveRel.gfp (F := BisimF)

theorem Bisim_gfp_coind {R : Rel} (h : R ≤ BisimF R) : R ≤ Bisim_gfp := by
  exact Telltale.Protocol.CoTypes.CoinductiveRel.coind (F := BisimF) h

theorem Bisim_gfp_unfold : Bisim_gfp ≤ BisimF Bisim_gfp := by
  exact Telltale.Protocol.CoTypes.CoinductiveRel.unfold (F := BisimF)

theorem Bisim_gfp_fold : BisimF Bisim_gfp ≤ Bisim_gfp := by
  exact Telltale.Protocol.CoTypes.CoinductiveRel.fold (F := BisimF)
