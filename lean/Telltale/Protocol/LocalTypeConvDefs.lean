...
import Telltale.Protocol.LocalTypeDB
import Telltale.Protocol.TypeContext

set_option linter.dupNamespace false

set_option linter.unnecessarySimpa false

/-! # Telltale.Protocol.LocalTypeConvDefs
...
namespace Telltale.Protocol.LocalTypeConv

open Telltale.Protocol.LocalTypeR
open Telltale.Protocol.LocalTypeDB
...
end Telltale.Protocol.LocalTypeConv
...
mutual
  def LocalTypeR.toDB? (ctx : Telltale.Protocol.LocalTypeConv.Context) :
      LocalTypeR → Option Telltale.Protocol.LocalTypeDB
    | .end => some .end
    | .var v =>
  ...
  def branchesToDB? (ctx : Telltale.Protocol.LocalTypeConv.Context) :
      List (Label × LocalTypeR) → Option (List (Label × Telltale.Protocol.LocalTypeDB))
    | [] => some []
    | (l, t) :: rest =>
...
namespace Telltale.Protocol.LocalTypeDB
...
mutual
  ...
  def fromDB? (ctx : Telltale.Protocol.LocalTypeConv.NameContext) :
      Telltale.Protocol.LocalTypeDB → Option LocalTypeR
    | .end => some LocalTypeR.end
    | .var n =>
        (Telltale.Protocol.LocalTypeConv.NameContext.get? ctx n).map LocalTypeR.var
    | .send p bs =>
        branchesFromDB? ctx bs |>.map (LocalTypeR.send p)
    | .recv p bs =>
        branchesFromDB? ctx bs |>.map (LocalTypeR.recv p)
    | .mu body =>
        let fresh := Telltale.Protocol.LocalTypeConv.NameContext.freshName ctx
        (body.fromDB? (NameOnlyContext.cons fresh ctx)).map (LocalTypeR.mu fresh)
  termination_by
    t => sizeOf t

  def branchesFromDB? (ctx : Telltale.Protocol.LocalTypeConv.NameContext) :
      List (Label × Telltale.Protocol.LocalTypeDB) → Option (List (Label × LocalTypeR))
    | [] => some []
    | (l, t) :: rest =>
...
mutual
  ...
  **Warning**: This function is unsafe and should only be used when you can guarantee that all
  ...
  def fromDB (ctx : Telltale.Protocol.LocalTypeConv.NameContext) :
      Telltale.Protocol.LocalTypeDB → LocalTypeR
    | .end => LocalTypeR.end
    | .var n =>
        match Telltale.Protocol.LocalTypeConv.NameContext.get? ctx n with
  ...
  def branchesFromDB (ctx : Telltale.Protocol.LocalTypeConv.NameContext) :
      List (Label × Telltale.Protocol.LocalTypeDB) → List (Label × LocalTypeR)
    | [] => []
    | (l, t) :: rest => (l, t.fromDB ctx) :: branchesFromDB ctx rest
  termination_by
    bs => sizeOf bs
end
end Telltale.Protocol.LocalTypeDB
