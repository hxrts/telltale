...
Solution Structure. Prove `d_env_foldl_append_comm` for trace accumulation.
Prove `lookup_g_append_left/right` for GEnv composition. These enable
...
theorem d_env_foldl_append_comm {ν : Type u} [VerificationModel ν]
  ...
  | nil =>
    ...
    -- D ++ ∅ = D
    have h : D ++ (∅ : DEnv) = D := d_env_union_empty_right D
    exact h.symm
  | cons hd tl ih =>
    simp only [List.foldl]
    ...
    -- -> D ++ (hd.traces ++ X) = D ++ (∅ ++ (hd.traces ++ X))
    -- -> D ++ (hd.traces ++ X) = D ++ (hd.traces ++ X)
    rw [d_env_union_assoc D hd.snd.traces (List.foldl (fun acc p => acc ++ p.snd.traces) ∅ tl)]
    rw [d_env_union_assoc (∅ : DEnv) hd.snd.traces
    ...
    rw [d_env_union_empty_left]

/-! ### `lookupG`/`updateG` append behavior -/

/-- lookupG in appended GEnv: searches left first. -/
theorem lookup_g_append_left {G1 G2 : GEnv} {e : Endpoint} {L : LocalType}
...
theorem lookup_g_append_right {G1 G2 : GEnv} {e : Endpoint}
    (h : lookupG G1 e = none) :
...
theorem update_g_append_right {G1 G2 : GEnv} {e : Endpoint} {L : LocalType}
...
theorem g_env_foldl_append_comm {ν : Type u} [VerificationModel ν]
...
theorem SessionStore.to_g_env_update_type {store : SessionStore ν} {e : Endpoint} {L : LocalType}
  ...
  | cons hd tl ih =>
      ...
      · simp only [SessionStore.updateType, hsid, ↓reduceIte]
        ...
        have hFoldUpd :
          ...
          exact g_env_foldl_append_comm (G := (st.updateType e L).localTypes) (store := tl)
        have hFoldOrig :
            List.foldl (fun acc p => acc ++ p.snd.localTypes) st.localTypes tl =
              st.localTypes ++ SessionStore.toGEnv tl := by
          unfold SessionStore.toGEnv
          exact g_env_foldl_append_comm (G := st.localTypes) (store := tl)
        simp only [List.nil_append]
        ...
        · have hLeft : lookupG (updateG st.localTypes e L ++ SessionStore.toGEnv tl) e' = some L := by
          ...
          have hRight : lookupG (updateG (st.localTypes ++ SessionStore.toGEnv tl) e L) e' = some L := by
            rw [he']
            exact lookup_g_update_g_eq (env := st.localTypes ++ SessionStore.toGEnv tl) (e := e) (L := L)
          exact hLeft.trans hRight.symm
        · have hR :
            lookupG (updateG (st.localTypes ++ SessionStore.toGEnv tl) e L) e' =
              lookupG (st.localTypes ++ SessionStore.toGEnv tl) e' :=
              lookup_g_update_g_ne (env := st.localTypes ++ SessionStore.toGEnv tl) (e := e) (e' := e') (L := L) he'
          have hL :
            ...
            · have hLeftUpd : lookupG (updateG st.localTypes e L) e' = none := by
                simpa [hLeft] using
/- ## Structured Block 1 -/
                  (lookup_g_update_g_ne (env := st.localTypes) (e := e) (e' := e') (L := L) he')
              rw [lookup_g_append_right (G1 := updateG st.localTypes e L) (G2 := SessionStore.toGEnv tl)
                (e := e') hLeftUpd]
              rw [lookup_g_append_right (G1 := st.localTypes) (G2 := SessionStore.toGEnv tl) (e := e') hLeft]
            · obtain ⟨T, hSome⟩ := Option.ne_none_iff_exists.mp hLeft
              ...
              have hLeftUpd : lookupG (updateG st.localTypes e L) e' = some T := by
                simpa [hSome'] using
                  (lookup_g_update_g_ne (env := st.localTypes) (e := e) (e' := e') (L := L) he')
              rw [lookup_g_append_left (G1 := updateG st.localTypes e L) (G2 := SessionStore.toGEnv tl)
                (e := e') (L := T) hLeftUpd]
              rw [lookup_g_append_left (G1 := st.localTypes) (G2 := SessionStore.toGEnv tl)
                (e := e') (L := T) hSome']
      ...
      · have hMem' : ∃ st', (e.sid, st') ∈ tl := by
        ...
        have hNotMemLeft : lookupG st.localTypes e = none :=
          SessionState.lookup_g_none_of_sid_ne hHead.2 hSidNe
        simp only [SessionStore.updateType, hsid, ↓reduceIte]
        simp only [SessionStore.toGEnv, List.foldl]
        have hFoldLeft :
          ...
          unfold SessionStore.toGEnv
          exact g_env_foldl_append_comm (G := st.localTypes) (store := SessionStore.updateType tl e L)
        have hFoldOrig :
            List.foldl (fun acc p => acc ++ p.snd.localTypes) st.localTypes tl =
              st.localTypes ++ SessionStore.toGEnv tl := by
          unfold SessionStore.toGEnv
        ...
        rw [update_g_append_right (G1 := st.localTypes) (G2 := SessionStore.toGEnv tl) (e := e) (L := L) hNotMemLeft]
        by_cases hLeft : lookupG st.localTypes e' = none
        · rw [lookup_g_append_right (G1 := st.localTypes) (G2 := SessionStore.toGEnv (SessionStore.updateType tl e L))
/- ## Structured Block 2 -/
            (e := e') hLeft]
          rw [lookup_g_append_right (G1 := st.localTypes) (G2 := updateG (SessionStore.toGEnv tl) e L)
            (e := e') hLeft]
          exact ih hMem' hCons'
        · obtain ⟨T, hSome⟩ := Option.ne_none_iff_exists.mp hLeft
          have hSome' : lookupG st.localTypes e' = some T := by
            simpa using hSome.symm
          rw [lookup_g_append_left (G1 := st.localTypes) (G2 := SessionStore.toGEnv (SessionStore.updateType tl e L))
            (e := e') (L := T) hSome']
          rw [lookup_g_append_left (G1 := st.localTypes) (G2 := updateG (SessionStore.toGEnv tl) e L)
            (e := e') (L := T) hSome']
...
theorem lookup_d_d_env_union_left {D1 D2 : DEnv} {e : Edge} {ts : Trace}
    (h : D1.find? e = some ts) :
    lookupD (D1 ++ D2) e = ts := by
  unfold lookupD
  have h' := d_env_union_find?_left (D1 := D1) (D2 := D2) (e := e) (ts := ts) h
  simp [h']

theorem lookup_d_d_env_union_right {D1 D2 : DEnv} {e : Edge}
    (h : D1.find? e = none) :
    lookupD (D1 ++ D2) e = lookupD D2 e := by
  unfold lookupD
  have h' := d_env_union_find?_right (D1 := D1) (D2 := D2) (e := e) h
  simp [h']

theorem lookup_d_update_d_union {D1 D2 : DEnv} {edge edge' : Edge} {ts : Trace} :
  ...
  · have hLeftFind : (updateD D1 edge ts).find? edge' = some ts := by
      rw [heq]
      exact find?_update_d_eq D1 edge ts
    have hLeft : lookupD (updateD D1 edge ts ++ D2) edge' = ts :=
      lookup_d_d_env_union_left hLeftFind
    have hRightEq : lookupD (updateD (D1 ++ D2) edge ts) edge' = ts := by
      rw [heq]
      exact lookup_d_update_eq (D1 ++ D2) edge ts
    calc
  ...
  · have hRight :
        ...
        lookup_d_update_neq (D1 ++ D2) edge edge' ts (Ne.symm heq)
    have hLeft :
      ...
      · have hFindUpd : (updateD D1 edge ts).find? edge' = none := by
          rw [find?_update_d_neq D1 edge edge' ts (Ne.symm heq)]
          exact hFind
        rw [lookup_d_d_env_union_right hFindUpd]
        rw [lookup_d_d_env_union_right hFind]
      · obtain ⟨ts', hSomeRaw⟩ := Option.ne_none_iff_exists.mp hFind
        ...
        have hSomeUpd : (updateD D1 edge ts).find? edge' = some ts' := by
          rw [find?_update_d_neq D1 edge edge' ts (Ne.symm heq)]
          exact hSome
        rw [lookup_d_d_env_union_left hSomeUpd]
        rw [lookup_d_d_env_union_left hSome]
    exact hLeft.trans (by simpa using hRight.symm)

/-! ### `toDEnv` Extensional Update Bridge -/

theorem SessionStore.to_d_env_update_trace {store : SessionStore ν} {edge : Edge} {ts : List ValType}
      ...
      · simp only [SessionStore.updateTrace, hsid, ↓reduceIte]
        ...
        have hFoldUpd :
          ...
          simpa [SessionStore.toDEnv] using
            (d_env_foldl_append_comm (D := (st.updateTrace edge ts).traces) (store := tl))
        have hFoldOrig :
            List.foldl (fun acc p => acc ++ p.snd.traces) st.traces tl =
              st.traces ++ SessionStore.toDEnv tl := by
          simpa [SessionStore.toDEnv] using
            (d_env_foldl_append_comm (D := st.traces) (store := tl))
        have hSeedUpd : (∅ : DEnv) ++ (st.updateTrace edge ts).traces = (st.updateTrace edge ts).traces :=
          d_env_union_empty_left (D := (st.updateTrace edge ts).traces)
        have hSeed : (∅ : DEnv) ++ st.traces = st.traces := d_env_union_empty_left (D := st.traces)
        rw [hSeedUpd, hSeed]
        rw [hFoldUpd, hFoldOrig]
...
/- ## Structured Block 3 -/
        ...
        have hFoldLeft :
          ...
          simpa [SessionStore.toDEnv] using
            (d_env_foldl_append_comm (D := ((∅ : DEnv) ++ st.traces)) (store := SessionStore.updateTrace tl edge ts))
        have hFoldOrig :
            List.foldl (fun acc p => acc ++ p.snd.traces) ((∅ : DEnv) ++ st.traces) tl =
              ((∅ : DEnv) ++ st.traces) ++ SessionStore.toDEnv tl := by
          simpa [SessionStore.toDEnv] using
            (d_env_foldl_append_comm (D := ((∅ : DEnv) ++ st.traces)) (store := tl))
        rw [hFoldLeft, hFoldOrig]
        have hEqLeft : ((∅ : DEnv) ++ st.traces) = st.traces := d_env_union_empty_left (D := st.traces)
        have hNoneLeft' : ((∅ : DEnv) ++ st.traces).find? edge = none := by
          simpa [hEqLeft] using hNoneLeft
        rw [update_d_d_env_union_right (D1 := ((∅ : DEnv) ++ st.traces)) (D2 := SessionStore.toDEnv tl)
...
theorem SessionStore.to_d_env_update_type {store : SessionStore ν} {e : Endpoint} {L : LocalType} :
  ...
  | cons hd tl ih =>
      ...
      · simp only [SessionStore.updateType, hsid, ↓reduceIte]
        ...
        have hSeed : (∅ : DEnv) ++ st.traces = st.traces := d_env_union_empty_left (D := st.traces)
        rw [hSeed]
        rw [d_env_foldl_append_comm (D := st.traces) (store := SessionStore.updateType tl e L)]
        rw [d_env_foldl_append_comm (D := st.traces) (store := tl)]
...
theorem SessionStore.to_g_env_update_trace {store : SessionStore ν} {edge : Edge} {ts : List ValType} :
  ...
  | cons hd tl ih =>
      ...
      · simp only [SessionStore.updateTrace, hsid, ↓reduceIte]
        ...
        rw [g_env_foldl_append_comm (G := st.localTypes) (store := SessionStore.updateTrace tl edge ts)]
        rw [g_env_foldl_append_comm (G := st.localTypes) (store := tl)]
...
theorem SessionStore.to_buffers_update_type {store : SessionStore ν} {e : Endpoint} {L : LocalType} :
...
theorem SessionStore.to_buffers_update_trace {store : SessionStore ν} {edge : Edge} {ts : List ValType} :
