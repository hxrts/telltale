...
private theorem lift_eq2_closure_comp_wf {a b : LocalTypeR}
...
theorem branches_rel_lift_comp_wf
    {bs cs : List BranchR}
  ...
  | cons hhead htail ih =>
      ...
      constructor
      · refine ⟨hhead.1, ?_⟩
        exact lift_eq2_closure_comp_wf hhead.2
...
theorem c_project_trans_rel_comp_wf_extend_right {a b c : LocalTypeR}
  ...
  exact ⟨c_project_trans_rel_comp_extend_right h1.1 h2 hWFa hWFb hWFc, hWFa, hWFc⟩

theorem c_project_trans_rel_comp_wf_extend_left {a b c : LocalTypeR}
    (h1 : EQ2 a b) (h2 : CProjectTransRelCompWF b c)
    (hWFa : LocalTypeR.WellFormed a)
    (hWFb : LocalTypeR.WellFormed b)
    (hWFc : LocalTypeR.WellFormed c) : CProjectTransRelCompWF a c := by
  -- Extend the underlying composition and reattach well-formedness.
  exact ⟨c_project_trans_rel_comp_extend_left h1 h2.1 hWFa hWFb hWFc, hWFa, hWFc⟩

/-! ## EQ2F Lifting Helpers -/

private theorem eq2_f_lift_comp_wf_send
  ...
  exact branches_rel_lift_comp_wf h hWFbs hWFcs

private theorem eq2_f_lift_comp_wf_recv
    {p q : String} {bs cs : List BranchR}
    {h : BranchesRel (EQ2_closure CProjectTransRelComp) bs cs}
...
private theorem eq2_f_lift_comp_wf_mu_left
  ...
  exact lift_eq2_closure_comp_wf h hWF_unfold_lt hWFc

private theorem eq2_f_lift_comp_wf_mu_right
  ...
  exact lift_eq2_closure_comp_wf h hWFa hWF_unfold_t

/-! ## EQ2F Mu/Mu Lifting -/

/- Helper: mu-mu case for EQ2F_lift_compWF. -/
private theorem eq2_f_lift_comp_wf_mu_mu
  ...
  have hWF_unfold_t : LocalTypeR.WellFormed (body_t.substitute v' (.mu v' body_t)) :=
    LocalTypeR.WellFormed.unfold hWFc
  exact ⟨
    lift_eq2_closure_comp_wf h.1 hWF_unfold_lt hWFc,
    lift_eq2_closure_comp_wf h.2 hWFa hWF_unfold_t
  ⟩

/-! ## EQ2F General Lifting -/

private theorem eq2_f_lift_comp_wf {lt t : LocalTypeR}
  ...
  case mu.mu =>
    exact eq2_f_lift_comp_wf_mu_mu h hWFa hWFc
  all_goals
    simp [EQ2F] at h ⊢
    first
    | exact eq2_f_lift_comp_wf_mu_right h hWFa hWFc
    | exact eq2_f_lift_comp_wf_mu_left h hWFa hWFc
    | exact ⟨h.1, eq2_f_lift_comp_wf_send (h := h.2) hWFa hWFc⟩
    | exact ⟨h.1, eq2_f_lift_comp_wf_recv (h := h.2) hWFa hWFc⟩

/-! ## Through-Mu Composition (EQ2 then CProjectTransRel) -/

theorem eq2_c_project_trans_rel_compose_through_mu_wf
  ...
  rcases c_project_trans_rel_source_mu (v := v) (body := body) (b := c) hrel with ⟨body_c, hc⟩
  subst hc
  have hcomp : EQ2F (EQ2_closure CProjectTransRelComp) a (.mu v body_c) := by
    ...
    | «end» | var _ | send _ _ | recv _ _ =>
        ...
        exact Or.inl (Or.inr (Or.inr (Or.inr ⟨.mu v body, .mu v body_c, heq, hrel,
          eq2_unfold_right (eq2_refl (.mu v body_c))⟩)))
  exact eq2_f_lift_comp_wf hcomp hWFa hWFc

/-! ## Through-Mu Composition (CProjectTransRel then EQ2) -/

theorem c_project_trans_rel_eq2_compose_through_mu_wf
    {a c : LocalTypeR} {v : String} {body : LocalTypeR}
  ...
  have h := c_project_trans_rel_eq2_compose_through_mu hrel heq hWFa hWFc
  exact eq2_f_lift_comp_wf h hWFa hWFc

/-! ## Three-Hop Composition in WF Closure -/

private theorem eq2_c_project_trans_rel_eq2_compose_wf
  ...
  have h := eq2_c_project_trans_rel_eq2_compose heq1 hrel heq2 hWFa hWFc
  exact eq2_f_lift_comp_wf h hWFa hWFc
...
This extends c_project_trans_rel_postfix to handle the composition cases:
- Base case: delegates to c_project_trans_rel_postfix
...
theorem c_project_trans_rel_comp_postfix_base
    {lt t : LocalTypeR}
...
theorem c_project_trans_rel_comp_postfix_chain
    {a c b b' : LocalTypeR}
    (heq_ab : EQ2 a b) (hrel_bb' : CProjectTransRel b b') (heq_b'c : EQ2 b' c)
    (hWFa : LocalTypeR.WellFormed a) (hWFc : LocalTypeR.WellFormed c) :
    EQ2F (EQ2_closure CProjectTransRelCompWF) a c := by
  -- 3-hop chain: lift via the compose lemma into the WF closure.
  exact eq2_c_project_trans_rel_eq2_compose_wf heq_ab hrel_bb' heq_b'c hWFa hWFc

/-! ## Prefix Mu Cases with WF Closure -/

theorem c_project_trans_rel_comp_postfix_prefix_mu_mu
  ...
  have hWF_unfold_t : LocalTypeR.WellFormed (body_t.substitute v' (.mu v' body_t)) :=
    LocalTypeR.WellFormed.unfold hWFc
  have hWFb : LocalTypeR.WellFormed b := c_project_trans_rel_wf_left hrel_bb'
  have hcomp_b : CProjectTransRelCompWF b (.mu v' body_t) :=
    c_project_trans_rel_comp_wf_of_c_project_trans_rel hrel_bb'
  have hcomp_lt_t : CProjectTransRelCompWF (.mu v body_lt) (.mu v' body_t) :=
    c_project_trans_rel_comp_wf_extend_left heq_ab hcomp_b hWFa hWFb hWFc
  constructor
  · have hcomp_left : CProjectTransRelCompWF
        (body_lt.substitute v (.mu v body_lt)) (.mu v' body_t) :=
      c_project_trans_rel_comp_wf_extend_left (eq2_unfold_left heq_ab) hcomp_b
...
theorem c_project_trans_rel_comp_postfix_prefix_mu_nonmu
  ...
  cases t with
  | mu v' body_t =>
      exact c_project_trans_rel_comp_postfix_prefix_mu_mu
...
theorem c_project_trans_rel_comp_postfix_prefix_nonmu_mu
...
theorem c_project_trans_rel_comp_postfix_prefix_var_var
  ...
  have hbase_f := c_project_trans_rel_postfix b (.var y) hrel_bb'
  cases b with
  | var z =>
      simp only [EQ2F] at heq_f hbase_f
      exact heq_f.trans hbase_f
  | mu _ _ =>
      exact eq2_c_project_trans_rel_compose_through_mu_wf heq_ab hrel_bb' hWFa hWFc
