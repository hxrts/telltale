import Choreography.Projection.Project.ImplTransRelComp
...
theorem c_project_trans_rel_source_end {b : LocalTypeR}
  ...
  have hne : g.allCommsNonEmpty = true := by
    ...
    exact hwf.1.1.2
  have htrans_end := c_project_end_trans_end g role hproj hne
  simpa [htrans_end] using htrans

/-- A CProjectTransRel from `.var v` can only target `.var v`. -/
theorem c_project_trans_rel_source_var {v : String} {b : LocalTypeR}
  ...
  have hne : g.allCommsNonEmpty = true := by
    simp only [GlobalType.wellFormed, Bool.and_eq_true] at hwf
    exact hwf.1.1.2
  have htrans_var := c_project_var_trans_var g role v hproj hne
  simpa [htrans_var] using htrans

/-- A CProjectTransRel from `.send` preserves the send constructor. -/
theorem c_project_trans_rel_source_send {p : String} {bs : List BranchR} {b : LocalTypeR}
  ...
  obtain ⟨gbs', htrans_send, _, _⟩ :=
    c_project_send_implies_trans_send g role p bs hproj hwf
  refine ⟨transBranches gbs' role, ?_⟩
  simpa [htrans_send] using htrans

/-- A CProjectTransRel from `.recv` preserves the recv constructor. -/
theorem c_project_trans_rel_source_recv {p : String} {bs : List BranchR} {b : LocalTypeR}
  ...
  obtain ⟨gbs', htrans_recv, _, _⟩ :=
    c_project_recv_implies_trans_recv g role p bs hproj hwf
  refine ⟨transBranches gbs' role, ?_⟩
  simpa [htrans_recv] using htrans

/-- A CProjectTransRel from `.mu` preserves the mu constructor. -/
theorem c_project_trans_rel_source_mu {v : String} {body : LocalTypeR} {b : LocalTypeR}
  ...
  obtain ⟨gbody, htrans_mu, _, _, _⟩ :=
    c_project_mu_implies_trans_mu g role v body hproj hne
...
private theorem GlobalType.well_formed_unfold (g : GlobalType)
    (hwf : g.wellFormed = true) : (GlobalType.unfold g).wellFormed = true := by
  -- Split on g and reuse the dedicated mu lemma.
  cases g with
  | mu t body =>
      simpa [GlobalType.unfold] using (well_formed_mu_unfold t body hwf)
  | «end» =>
...
private theorem GlobalType.well_formed_unfold_iter (g : GlobalType)
...
private theorem GlobalType.well_formed_unfold_rec (g : GlobalType)
  ...
  | succ n ih =>
      ...
      have hwf' :
        ...
        ih (g := g) hwf
      simpa using (GlobalType.well_formed_unfold
...
private theorem well_formed_unfold_rec (g : GlobalType) (n : Nat) (hwf : g.wellFormed = true) :
    (unfoldRec g n).wellFormed = true := by
  -- Reduce to the recursor lemma.
  simpa [unfoldRec] using GlobalType.well_formed_unfold_rec g n hwf
...
private theorem trans_unfold_eq2_mu_unguarded
  ...
  have heq : EQ2 ((trans body role).substitute t .end) .end :=
    Choreography.Harmony.SubstEndUnguarded.subst_end_unguarded_eq2_end
...
private theorem trans_unfold_eq2_mu (t : String) (body : GlobalType) (role : String)
  ...
  have hclosed : (.mu t body : GlobalType).isClosed = true :=
    GlobalType.is_closed_of_well_formed (.mu t body) hwf
...
private theorem trans_unfold_to_trans (g : GlobalType) (role : String)
  ...
  have hWFc : LocalTypeR.WellFormed (trans g role) :=
    trans_well_formed_of_well_formed g role hwf
  ...
  have hWFa : LocalTypeR.WellFormed (trans (GlobalType.unfold g) role) :=
    trans_well_formed_of_well_formed (GlobalType.unfold g) role hwf'
...
private theorem trans_unfold_iter_eq2 (g : GlobalType) (role : String)
  ...
  | succ n ih =>
      ...
      have hwf' : (unfoldRec g n).wellFormed = true := well_formed_unfold_rec g n hwf
      have hstep :
      ...
      have hWF_mid : LocalTypeR.WellFormed (trans (unfoldRec g n) role) :=
        trans_well_formed_of_well_formed (unfoldRec g n) role hwf'
      have hWF_left : LocalTypeR.WellFormed (trans (GlobalType.unfold (unfoldRec g n)) role) :=
        trans_well_formed_of_well_formed (GlobalType.unfold (unfoldRec g n)) role
          (GlobalType.well_formed_unfold (unfoldRec g n) hwf')
...
private theorem trans_full_unfold_iter_eq2 (g : GlobalType) (role : String)
    (hwf : g.wellFormed = true) :
...
private theorem proj_eq_full (g : GlobalType) (role : String)
  ...
  exact trans_full_unfold_iter_eq2 g role hwf
...
private theorem eq2_unfold_left_full_unfold {a b : LocalTypeR} (h : EQ2 a b) :
...
private theorem eq2_unfold_right_full_unfold {a b : LocalTypeR} (h : EQ2 a b) :
...
private theorem eq2_closure_extend_right
  ...
  cases hxy with
  | inl hr => exact Or.inl (c_project_trans_rel_comp_extend_right hr hyz hWFx hWFy hWFz)
...
private theorem c_project_trans_rel_eq2_compose_through_mu_mu_mu_right
...
/-- Composing EQ2 → CProjectTransRel → EQ2 (3-hop) satisfies EQ2F.
    Used to discharge the 3-hop case in c_project_trans_rel_comp_postfix. -/
theorem eq2_c_project_trans_rel_eq2_compose
  ...
  have hne : g.allCommsNonEmpty = true := by
    simp only [GlobalType.wellFormed, Bool.and_eq_true] at hwf
    exact hwf.1.1.2
  have htrans_eq : trans g role = b := trans_eq_of_c_project g role b hproj hne
  have hb' : b' = b := by
  ...
  exact eq2_f_mono (fun _ _ h => Or.inr h) hF

/-! ## Mu-Compose Case Split Helpers -/

/-- Helper: mu/mu target case for CProjectTransRel_EQ2_compose_through_mu. -/
private theorem c_project_trans_rel_eq2_compose_through_mu_mu_mu
    {av v cv : String} {abody body cbody : LocalTypeR}
    (hmid_left :
      EQ2_closure CProjectTransRelComp
  ...
  have hright :
    ...
    c_project_trans_rel_eq2_compose_through_mu_mu_mu_right
...
private theorem c_project_trans_rel_eq2_compose_through_mu_mu_nonmu
...
private theorem c_project_trans_rel_eq2_compose_through_mu_mu
  ...
  have hmid : EQ2F (EQ2_closure CProjectTransRelComp) (.mu av abody) (.mu v body) :=
    c_project_trans_rel_postfix _ _ hrel
  simp only [EQ2F] at hmid
  obtain ⟨hmid_left, hmid_right⟩ := hmid
  have hWF_mu : LocalTypeR.WellFormed (.mu v body) := c_project_trans_rel_wf_right hrel
  ...
  cases c with
  | mu cv cbody =>
      exact c_project_trans_rel_eq2_compose_through_mu_mu_mu
        hmid_left hmid_right heq hWFa hWF_unfold_a hWF_mu hWF_unfold_mu hWFc
  | «end» | var _ | send _ _ | recv _ _ =>
      simpa [EQ2F] using
        (c_project_trans_rel_eq2_compose_through_mu_mu_nonmu
...
theorem c_project_trans_rel_eq2_compose_through_mu
  ...
  cases a with
  | mu av abody =>
      exact c_project_trans_rel_eq2_compose_through_mu_mu hrel heq hWFa hWFc
  | «end» =>
      cases (c_project_trans_rel_source_end (b := .mu v body) hrel)
  | var x =>
      cases (c_project_trans_rel_source_var (v := x) (b := .mu v body) hrel)
  | send p bs =>
      rcases c_project_trans_rel_source_send (p := p) (bs := bs) (b := .mu v body) hrel with ⟨cs, hEq⟩
      cases hEq
  | recv p bs =>
      rcases c_project_trans_rel_source_recv (p := p) (bs := bs) (b := .mu v body) hrel with ⟨cs, hEq⟩
      cases hEq
