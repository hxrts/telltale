...
/-! # SessionCoTypes.Bisim.Congruence
...
theorem dual_compatible : Compatible LocalTypeR.dual := by
  ...
  | eq_send hx hy hbr =>
      rename_i p bsa bsb
      have hx' : CanRecv x.dual p (dualBranches bsa) := CanSend.dual hx
...
theorem substitute_preserves_UnfoldsToEnd {a : LocalTypeR} {var : String} {repl : LocalTypeR}
  ...
  · intro t body hinner ih repl hWFa hWFrepl
    ...
    -- hWFa : LocalTypeR.WellFormed (.mu t body) implies .mu t body is closed.
    left
