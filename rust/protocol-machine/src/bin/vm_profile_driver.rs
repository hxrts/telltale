...
fn usage_and_exit() -> ! {
    eprintln!(
        "usage: cargo run -p telltale-protocol-machine --release --bin vm_profile_driver -- \\\n\
    ...
    for _ in 0..iterations {
        ...
        checksum ^= black_box(vm.trace().len())
            ^ black_box(bool_to_usize(matches!(
                status,
                telltale_protocol_machine::ProtocolMachineRunStatus::Stuck
            )));
    }
    checksum
}
...
fn profile_scheduler_many_paused_run_only(yields_per_role: usize) -> usize {
    ...
    black_box(vm.trace().len())
        ^ black_box(bool_to_usize(matches!(
            status,
            telltale_protocol_machine::ProtocolMachineRunStatus::MaxRoundsExceeded
        )))
}
...
fn profile_send_recv_replay_nullifier(iterations: usize) -> usize {
    ...
    black_box(vm.trace().len())
        ^ black_box(bool_to_usize(matches!(
            status,
            telltale_protocol_machine::ProtocolMachineRunStatus::AllDone
        )))
}
