...
use telltale_protocol_machine::buffer::BufferConfig;
use telltale_protocol_machine::coroutine::{CoroStatus, Fault};
...
use telltale_protocol_machine::output_condition::OutputConditionHint;
use telltale_protocol_machine::session::{
...
fn single_role_end_image(
    program: Vec<telltale_protocol_machine::instr::Instr>,
...
impl EffectHandler for KnowledgePayloadHandler {
    fn handle_send(
        ...
        _partner: &str,
        _label: &str,
        _state: &[telltale_protocol_machine::coroutine::Value],
    ) -> EffectResult<telltale_protocol_machine::coroutine::Value> {
        EffectResult::success(telltale_protocol_machine::coroutine::Value::Unit)
    }

    fn send_decision(&self, input: SendDecisionInput<'_>) -> EffectResult<SendDecision> {
        EffectResult::success(SendDecision::Deliver(telltale_protocol_machine::coroutine::Value::Prod(
            Box::new(telltale_protocol_machine::coroutine::Value::Endpoint(Endpoint {
                sid: input.sid,
                role: input.role.to_string(),
            })),
            Box::new(telltale_protocol_machine::coroutine::Value::Str("secret".to_string())),
        )))
    }
    ...
    fn handle_recv(
        ...
        _partner: &str,
        _label: &str,
        _state: &mut Vec<telltale_protocol_machine::coroutine::Value>,
        _payload: &telltale_protocol_machine::coroutine::Value,
    ) -> EffectResult<()> {
    ...
    fn handle_choose(
        ...
        match labels.first().cloned() {
            ...
            None => EffectResult::failure(telltale_protocol_machine::effect::EffectFailure::invalid_input(
                "no labels available",
            )),
        }
    }
...
fn test_lean_send_receive_signature_verification() {
    ...
    signed.signature.signer = telltale_protocol_machine::verification::VerifyingKey([0_u8; 32]);
    let _ = sess
    ...
    assert_matches!(
        result,
        Err(telltale_protocol_machine::ProtocolMachineError::Fault {
...
fn test_lean_acquire_release_guard_behavior() {
    use telltale_protocol_machine::instr::Instr;
...
fn test_lean_invoke_and_output_condition_hint_behavior() {
    use telltale_protocol_machine::instr::{Instr, InvokeAction};
...
fn test_lean_authority_timeout_and_cancellation_trace_behavior() {
    ...
    assert!(
        matches!(step, telltale_protocol_machine::ProtocolMachineStepResult::Stuck),
...
fn test_lean_authority_evidence_rejection_behavior() {
    ...
    let forged = telltale_protocol_machine::session::ReadinessWitness {
    ...
    assert!(
        matches!(
            forged_err,
            telltale_protocol_machine::session::OwnershipError::InvalidWitness { .. }
        ),
    ...
    assert!(
        matches!(
            reused,
            telltale_protocol_machine::session::OwnershipError::WitnessConsumed { .. }
        ),
...
fn test_lean_abort_policy_is_deterministic_and_scoped() {
    ...
    let run_once = || {
        ...
        assert_matches!(
            ...
            Ok(telltale_protocol_machine::ProtocolMachineStepResult::Continue)
        ); // set
        assert_matches!(
            vm.step(&PassthroughHandler),
            Ok(telltale_protocol_machine::ProtocolMachineStepResult::Continue)
...
fn test_lean_schedule_confluence() {
    use telltale_protocol_machine::SchedPolicy;
...
fn test_lean_wf_pc_bounds() {
    ...
    for _ in 0..50 {
        ...
        match vm.step(&handler) {
            Ok(
                telltale_protocol_machine::ProtocolMachineStepResult::AllDone
                | telltale_protocol_machine::ProtocolMachineStepResult::Stuck,
            ) => break,
            Ok(telltale_protocol_machine::ProtocolMachineStepResult::Continue) => {}
            Err(_) => break,
        }
