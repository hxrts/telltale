...
use cfg_if::cfg_if;
use telltale_protocol_machine::architecture::{EngineRole, CANONICAL_ENGINE, ENGINE_OWNERSHIP};

cfg_if! {
    if #[cfg(feature = "multi-thread")] {
        use telltale_protocol_machine::ThreadedProtocolMachine;
        use telltale_protocol_machine::trace::{normalize_trace, with_tick};
        use telltale_protocol_machine::{ProtocolMachineConfig, ProtocolMachine};
...
cfg_if! {
    if #[cfg(feature = "multi-thread")] {
        ...
        fn canonicalize_event(event: &telltale_protocol_machine::ObsEvent) -> telltale_protocol_machine::ObsEvent {
            match event {
                telltale_protocol_machine::ObsEvent::OutputConditionChecked {
                    predicate_ref,
                    witness_ref,
                    passed,
                    ..
