...
use telltale_protocol_machine::{FlowPolicy, FlowPredicate, ProtocolMachineConfig};
...
fn named_strict_profiles_encode_explicit_runtime_modes() {
    ...
    assert_eq!(minimal.determinism_mode, telltale_protocol_machine::DeterminismMode::Full);
    assert_eq!(
        minimal.threaded_round_semantics,
        telltale_protocol_machine::ThreadedRoundSemantics::CanonicalOneStep
    );
    assert_eq!(
        minimal.effect_trace_capture_mode,
        telltale_protocol_machine::EffectTraceCaptureMode::Disabled
    );
    assert_eq!(
        verified.communication_replay_mode,
        telltale_protocol_machine::CommunicationReplayMode::Nullifier
    );
    assert_eq!(
        verified.payload_validation_mode,
        telltale_protocol_machine::PayloadValidationMode::StrictSchema
    );
    assert_eq!(
        observable.effect_trace_capture_mode,
        telltale_protocol_machine::EffectTraceCaptureMode::Full
    );
}
...
fn vm_new_rejects_invalid_config() {
    ...
    drop(telltale_protocol_machine::ProtocolMachine::new(cfg));
}
...
cfg_if! {
    if #[cfg(feature = "multi-thread")] {
        ...
        fn threaded_vm_rejects_invalid_config() {
            ...
            drop(telltale_protocol_machine::ThreadedProtocolMachine::with_workers(cfg, 2));
        }
    }
}
