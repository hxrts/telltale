...
fn fault_name(err: &ProtocolMachineError) -> Option<&'static str> {
    match err {
        ProtocolMachineError::Fault { fault, .. } => Some(match fault {
            telltale_protocol_machine::coroutine::Fault::TypeViolation { .. } => "type_violation",
            telltale_protocol_machine::coroutine::Fault::UnknownLabel { .. } => "unknown_label",
            telltale_protocol_machine::coroutine::Fault::Transfer { .. } => "transfer_fault",
            telltale_protocol_machine::coroutine::Fault::OutOfRegisters => "out_of_registers",
            telltale_protocol_machine::coroutine::Fault::Close { .. } => "close_fault",
            telltale_protocol_machine::coroutine::Fault::Speculation { .. } => "spec_fault",
            _ => "other_fault",
        }),
        _ => None,
    }
