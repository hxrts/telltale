...
use telltale_protocol_machine::session::SessionStore;
...
impl EffectHandler for BenchHandler {
    ...
    fn handle_send(
        ...
        telltale_protocol_machine::effect::EffectResult::success(Value::Unit)
    }
    ...
    fn step(&self, _role: &str, _state: &mut Vec<Value>) -> telltale_protocol_machine::effect::EffectResult<()> {
        telltale_protocol_machine::effect::EffectResult::success(())
    }
}
    ...
    for idx in 0..iterations {
        ...
        for coro_id in coro_ids {
            vm.coroutine_mut(coro_id).expect("coroutine exists").status =
                telltale_protocol_machine::CoroStatus::Done;
        }
...
pub(crate) fn signing_fixture() -> (Value, telltale_protocol_machine::Signature) {
    let endpoint = Endpoint {
        sid: 7,
        role: "A".to_string(),
    };
    let payload = Value::Prod(Box::new(Value::Nat(7)), Box::new(Value::Bool(true)));
    let key = telltale_protocol_machine::verification::signing_key_for_endpoint(&endpoint);
    let signature = telltale_protocol_machine::sign_value(&payload, &key);
    (payload, signature)
}
