...
use serde::Serialize;
use telltale_protocol_machine::communication_replay::{
...
use telltale_protocol_machine::session::Edge;
use telltale_protocol_machine::verification::{
...
fn legacy_identity_build(
    ...
    CommunicationIdentity {
        domain_tag: telltale_protocol_machine::COMM_IDENTITY_DOMAIN_TAG.to_string(),
        sid: edge.sid,
...
pub(crate) fn bench_verification_and_allocations(c: &mut Criterion) {
    ...
    c.bench_function("vm_sign_value_only", |b| {
        ...
        b.iter(|| {
            black_box(telltale_protocol_machine::sign_value(
                black_box(&payload),
                black_box(&key),
            ))
        });
    ...
    c.bench_function("vm_verify_signed_value_only", |b| {
        ...
        b.iter(|| {
            black_box(telltale_protocol_machine::verify_signed_value(
                black_box(&payload),
                black_box(&signature),
                black_box(&verifying),
            ))
    ...
    c.bench_function("vm_auth_tree_append_only", |b| {
        let (payload, signature) = signing_fixture();
        let signed = telltale_protocol_machine::buffer::SignedValue {
            payload,
            signature,
            sequence_no: 1,
        };
