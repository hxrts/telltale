...
#[derive(Clone, Debug, Default)]
pub(crate) struct UnstableOrderDomain {
    pub(crate) edges: BTreeMap<u8, Vec<(u8, &'static str, u64)>>,
    pub(crate) heuristics: BTreeMap<(u64, u8), u64>,
    pub(crate) counter: Arc<AtomicUsize>,
}

impl SearchDomain for UnstableOrderDomain {
    type Node = u8;
    type EdgeMeta = &'static str;
    type Cost = u64;
    type GraphEpoch = u64;
    ...
    fn successors(
        ...
        if let Some(edges) = self.edges.get(node) {
            let mut unstable = edges.clone();
            if self.counter.fetch_add(1, Ordering::SeqCst) % 2 == 1 {
                unstable.reverse();
            }
            out.extend(unstable);
        }
