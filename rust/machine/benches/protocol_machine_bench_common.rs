...
pub(crate) fn signing_fixture() -> (Value, telltale_machine::Signature) {
    let endpoint = Endpoint {
        sid: 7,
        role: "A".to_string(),
    };
    let payload = Value::Prod(Box::new(Value::Nat(7)), Box::new(Value::Bool(true)));
    let key = telltale_machine::verification::signing_key_for_endpoint(&endpoint);
    let signature = telltale_machine::sign_value(&payload, &key);
    (payload, signature)
}
