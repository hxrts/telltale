...
use serde::Serialize;
use telltale_machine::communication_replay::{
...
fn legacy_identity_build(
    ...
    CommunicationIdentity {
        domain_tag: telltale_machine::COMM_IDENTITY_DOMAIN_TAG.to_string(),
        sid: edge.sid,
...
pub(crate) fn bench_verification_and_allocations(c: &mut Criterion) {
    ...
    c.bench_function("vm_sign_value_only", |b| {
        ...
        b.iter(|| {
            black_box(telltale_machine::sign_value(
                black_box(&payload),
                black_box(&key),
            ))
        });
    ...
    c.bench_function("vm_verify_signed_value_only", |b| {
        ...
        b.iter(|| {
            black_box(telltale_machine::verify_signed_value(
                black_box(&payload),
                black_box(&signature),
                black_box(&verifying),
            ))
