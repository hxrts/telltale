...
/// Parse an invariant/property declaration into the canonical property model.
pub fn parse_property(s: &str) -> Result<Property, String> {
    let name = s.trim();
    match name {
        "no_faults" => return Ok(Property::NoFaults),
        "simplex" => return Ok(Property::Simplex),
        _ => {}
    }
