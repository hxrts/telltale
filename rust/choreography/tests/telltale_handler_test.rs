...
// Integration tests for TelltaleHandler with SimpleChannel
...
impl telltale::Message<Box<dyn std::any::Any + Send>> for TestMessage {
...
async fn test_simple_send_recv() {
    ...
    let mut alice_endpoint = TelltaleEndpoint::new(TestRole::Alice);
    let mut bob_endpoint = TelltaleEndpoint::new(TestRole::Bob);
    ...
    // Create handlers
    let mut alice_handler = TelltaleHandler::<TestRole, TestMessage>::new();
    let mut bob_handler = TelltaleHandler::<TestRole, TestMessage>::new();
...
async fn test_dynamic_session_flow() {
    ...
    alice_endpoint.register_session(
        TestRole::Bob,
        TelltaleSession::from_simple_channel(alice_channel),
    );
    bob_endpoint.register_session(
        TestRole::Alice,
        TelltaleSession::from_simple_channel(bob_channel),
    );
