...
Core theorem anchors and their source locations are listed in Table~\ref{tab:p2-anchor-registry} (Appendix~\ref{app:p2-anchor-registry}).

\begin{proposition}[Mechanization Coverage Soundness]
	\label{prop:coverage}
	For any anchor in Table~\ref{tab:p2-anchor-registry}, if the corresponding Appendix~\ref{app:reproducibility} checks pass, then there exists a mechanized theorem object within this paper's assumption scope.
\end{proposition}

\begin{proof}[Proof sketch]
	Anchors in Table~\ref{tab:p2-anchor-registry} are defined by explicit theorem names and file paths. Appendix~\ref{app:reproducibility} rebuild checks certify that these declarations resolve and typecheck under the pinned toolchain. Therefore a passing check witnesses existence of a theorem object for each anchor.
\end{proof}
...
\begin{table}[H]
	...
	\begin{tabularx}{\columnwidth}{@{}L{0.42\columnwidth}L{0.53\columnwidth}@{}}
		...
		\texttt{total\_measure\_decreasing} & \texttt{Runtime/Proofs/}\mbox{\texttt{Lyapunov.lean}} \\
		...
		\texttt{regularTypeEqDecide\_spec} & \texttt{SessionCoTypes/Coinductive/\allowbreak BisimDecidable/}\mbox{\texttt{*.lean}} \\
		\texttt{crash\_tolerance\_iff} & \texttt{Protocol/}\mbox{\texttt{CrashTolerance.lean}} \\
		\texttt{branching\_iff\_chromatic\_capacity} & \texttt{Protocol/BufferBoundedness/\allowbreak}\mbox{\texttt{PhaseSharpness.lean}} \\
		\texttt{phase\_transition\_sharp} & \texttt{Protocol/BufferBoundedness/\allowbreak}\mbox{\texttt{PhaseSharpness.lean}} \\
		\texttt{byzantineSafety\_exact\_of\_profile} & \texttt{Runtime/Proofs/Adapters/Distributed/\allowbreak}\mbox{\texttt{EnvelopeTheorems.lean}} \\
...
\begin{table}[H]
	...
	\begin{tabularx}{\columnwidth}{@{}L{0.22\columnwidth}L{0.09\columnwidth}L{0.40\columnwidth}L{0.19\columnwidth}@{}}
		...
		Thm.~\ref{thm:algorithmic}     & \S5 & Assump.~\ref{assump:p2-regular}      & \texttt{reachable*} \\
			...
			Prop.~\ref{prop:coverage}      & \S9 & anchor registry + App.~\ref{app:reproducibility} checks & \S9 \\
		\bottomrule
	\end{tabularx}
	}
