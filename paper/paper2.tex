...
\begin{abstract}
	We prove that quotient dynamics for asynchronous MPST are computable in two coordinated senses.
	The first is quantitative and uses a weighted Lyapunov measure. The second is algorithmic and
	uses finite-reachability decision schemas over regular fragments. The weighted measure is
...
Related liveness lines often emphasize qualitative progress in session-typed settings (Honda et al., 2008, and Caires and Pfenning, 2010). Related mechanized lines expose proof brittleness and motivate reusable proof architecture (Tirore et al., 2025). Figure~1 records the proof split used in this paper: one shared regularity kernel induces both the quantitative branch (descent and bounds) and the algorithmic branch (finite exploration and deciders).
...
\textbf{Running example.} Protocol (as in the Coherence paper):
