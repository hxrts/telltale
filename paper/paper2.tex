...
\label{app:p2-anchor-registry}

Table~\ref{tab:p2-anchor-registry} maps theorem anchors to their source files.
...
\begin{table}[H]
	...
	\begin{tabularx}{\columnwidth}{@{}L{0.475\columnwidth}L{0.475\columnwidth}@{}}
		...
		\midrule
		\texttt{weightedMeasure} & \texttt{Runtime/Proofs/\allowbreak WeightedMeasure/}\mbox{\texttt{*.lean}} \\
		\texttt{total\_measure\_decreasing} & \texttt{Runtime/Proofs/WeightedMeasure/}\mbox{\texttt{TotalBound.lean}} \\
		\texttt{reachable\_triples\_finite} & \texttt{SessionCoTypes/AsyncSubtyping/}\mbox{\texttt{*.lean}} \\
		\texttt{async\_subtype\_decidable} & \texttt{SessionCoTypes/AsyncSubtyping/}\mbox{\texttt{*.lean}} \\
		\texttt{regular\_type\_eq\_decide\_spec} & \texttt{SessionCoTypes/Coinductive/\allowbreak BisimDecidable/}\mbox{\texttt{*.lean}} \\
		\texttt{crash\_tolerance\_iff} & \texttt{Protocol/}\mbox{\texttt{CrashTolerance.lean}} \\
		\texttt{branching\_iff\_chromatic\_capacity} & \texttt{Protocol/}\mbox{\texttt{SpatialBranching.lean}} \\
		\texttt{phase\_transition\_sharp} & \texttt{Protocol/BufferBoundedness/\allowbreak}\mbox{\texttt{PhaseSharpness.lean}} \\
		\texttt{byzantine\_safety\_exact\_of\_profile} & \texttt{Runtime/Proofs/Adapters/Distributed/\allowbreak}\mbox{\texttt{EnvelopeTheorems.lean}} \\
		\texttt{canOperateUnderByzantineEnvelope} & \texttt{Runtime/Proofs/TheoremPack/}\mbox{\texttt{*.lean}} \\
		\bottomrule
	\end{tabularx}
	\caption{Anchor registry.}
	\label{tab:p2-anchor-registry}
\end{table}
...
\begin{enumerate}
	\item Build module families listed in Appendix~\ref{app:p2-anchor-registry}.
	\item Run \texttt{just escape} and \texttt{just verify-protocols}.
	\item Confirm Appendix~\ref{app:p2-anchor-registry} theorem anchors resolve and typecheck.
\end{enumerate}
...
\begin{table}[H]
	...
	\begin{tabularx}{\columnwidth}{@{}L{0.22\columnwidth}L{0.09\columnwidth}L{0.40\columnwidth}L{0.19\columnwidth}@{}}
		\toprule
		\thead{Claim} & \thead{Sec.} & \thead{Assumption} & \thead{Anchor} \\
		\midrule
		Thm.~\ref{thm:quantitative}    & \S4 & Assump.~\ref{assump:p2-quantitative} & \texttt{weighted*} \\
		Thm.~\ref{thm:algorithmic}     & \S5 & Assump.~\ref{assump:p2-regular}      & \texttt{reachable*} \\
			Thm.~\ref{thm:crash}           & \S6 & Assump.~\ref{assump:p2-crash}        & \texttt{crash\_*} \\
			Thm.~\ref{thm:byzantine}       & \S6 & Assump.~\ref{assump:p2-byzantine}    & \texttt{byzantine*} \\
			Cor.~\ref{cor:converse}        & \S6 & Thm.~\ref{thm:byzantine} + dropped class & \texttt{byzantine*} \\
			Cor.~\ref{cor:productive}      & \S7 & Thm.~\ref{thm:quantitative} premises & \texttt{weighted*} \\
			Cor.~\ref{cor:scheduler}       & \S7 & Thm.~\ref{thm:quantitative} + sched. & \texttt{total\_*} \\
			Cor.~\ref{cor:capacity}        & \S7 & regular + threshold assumptions       & \texttt{phase\_*} \\
			Thm.~\ref{thm:app-p2-branching} & App.\ C & regular + branch profile & \texttt{branching*} \\
			Prop.~\ref{prop:vm-bridge}     & \S6 & pack + adherence                      & \texttt{canOperate*} \\
		\bottomrule
	\end{tabularx}
	}
