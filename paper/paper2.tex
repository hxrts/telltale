...
Related liveness lines often emphasize qualitative progress in session-typed settings (Honda et al., 2008, and Caires and Pfenning, 2010). Related mechanized lines expose proof brittleness and motivate reusable proof architecture (Tirore et al., 2025). Equation~\ref{eq:p2-proof-split} records the proof split used in this paper: one shared regularity kernel induces both the quantitative branch (descent and bounds) and the algorithmic branch (finite exploration and deciders).
...
Table~\ref{tab:p2-notation} fixes notation used in this paper.
...
The weighted-measure decomposition used by Theorem~1 is:
\begin{equation}
...
Coinductive boundary note: effect-level transport is restricted to the witness-carrying rational subclass via \texttt{rationalEffect\_transport\_bridge}; witness-check soundness/completeness is provided by \texttt{checkRationalEffectWitness\_sound} and \texttt{checkRationalEffectWitness\_complete}; strict outside non-transportability is witnessed by \texttt{strict\_boundary\_witness\_effect}. These do not alter Theorem~2 assumptions and are used in the \emph{Harmony from Coherence in Asynchronous MPST} boundary layer.
...
Dependency sketch:
\begin{enumerate}
	\item Assumption Block~\ref{assump:p2-quantitative} gives Theorem~1 and Corollaries~1--2.
	\item Assumption Block~\ref{assump:p2-regular} gives Theorem~2 and the subtyping/equivalence deciders.
	\item Assumption Block~\ref{assump:p2-crash} gives Theorem~3.
	\item Assumption Block~\ref{assump:p2-byzantine} gives Theorem~4, Corollary~4.1, and Proposition~2.
\end{enumerate}
...
\begin{table}[tbp]
	...
	\begin{tabularx}{\columnwidth}{@{}L{0.22\columnwidth}L{0.42\columnwidth}L{0.3\columnwidth}@{}}
		...
		\midrule
		Weighted dyn.  & \nolinkurl{WeightedMeasure/*.lean}, \nolinkurl{Lyapunov.lean}        & \texttt{weightedMeasure}, \texttt{total\_measure\_decr} \\
		Decision       & \nolinkurl{AsyncSubtyping/*.lean}, \nolinkurl{BisimDecidable/*.lean} & \texttt{reachable\_triples\_finite}                      \\
		Protocol       & \nolinkurl{CrashTolerance.lean}, \nolinkurl{PhaseSharpness.lean}     & \texttt{crash\_tolerance\_iff}                           \\
		Byzantine      & \nolinkurl{EnvelopeTheorems.lean}, \nolinkurl{TheoremPack/*.lean}    & \texttt{byzantineSafety\_exact}                          \\
		\bottomrule
	\end{tabularx}
...
\begin{proposition}[Mechanization Coverage Soundness]
	\label{prop:coverage}
	For any claim class $c$ in Table~\ref{tab:p2-claim-artifact-map}, if $c$ is marked covered and the corresponding Appendix~F checks pass, then there exists a mechanized theorem object for $c$ within this paper's assumption scope.
\end{proposition}

\begin{proof}[Proof sketch]
	Coverage labels in Table~\ref{tab:p2-claim-artifact-map} are defined by explicit module-and-theorem anchors. Appendix~F rebuild checks certify that these declarations resolve and typecheck under the pinned toolchain. Therefore a passing check witnesses existence of a theorem object for each covered claim class.
\end{proof}
...
Theorem~1 and Corollaries~1--2 are exactly Propositions~A.4--A.5 under Assumption Block~\ref{assump:p2-quantitative}.
...
\begin{table}[tbp]
	...
	\begin{tabularx}{\columnwidth}{@{}L{0.24\columnwidth}L{0.42\columnwidth}L{0.28\columnwidth}@{}}
		...
		\midrule
		Quant.\ dynamics & \nolinkurl{WeightedMeasure/*.lean}, \nolinkurl{Lyapunov.lean}        & \texttt{weightedMeasure}            \\
		Decidability     & \nolinkurl{AsyncSubtyping/*.lean}, \nolinkurl{BisimDecidable/*.lean} & \texttt{reachable\_triples\_finite} \\
		Crash/branch     & \nolinkurl{CrashTolerance.lean}, \nolinkurl{PhaseSharpness.lean}     & \texttt{crash\_tolerance\_iff}      \\
		Byzantine        & \nolinkurl{EnvelopeTheorems*.lean}, \nolinkurl{TheoremPack/*.lean}   & \texttt{byzantineSafety\_exact}     \\
		\bottomrule
...
\begin{table}[!h]
	...
	\begin{tabularx}{\columnwidth}{@{}L{0.22\columnwidth}L{0.1\columnwidth}L{0.32\columnwidth}L{0.3\columnwidth}@{}}
		...
		\midrule
		Theorem 1      & \S4           & Assump.~\ref{assump:p2-quantitative} & \nolinkurl{WeightedMeasure/*.lean} \\
		Theorem 2      & \S5           & Assump.~\ref{assump:p2-regular}      & \nolinkurl{AsyncSubtyping/*.lean}  \\
		Theorem 3      & \S6           & Assump.~\ref{assump:p2-crash}        & \nolinkurl{CrashTolerance.lean}    \\
		Theorem 4      & \S6           & Assump.~\ref{assump:p2-byzantine}    & \nolinkurl{EnvelopeTheorems.lean}  \\
		Cor.\ 4.1      & \S6           & Thm 4 + dropped     & Section 6              \\
		Cor.\ 1        & \S7           & Thm 1 premises      & derived in \S7         \\
		Cor.\ 2        & \S7           & Thm 1 + sched.      & derived in \S7         \\
		Cor.\ 3        & \S7           & regular + thresh.   & \nolinkurl{PhaseSharpness.lean}    \\
		Prop.\ 2       & \S6           & pack + adhere       & \nolinkurl{TheoremPack/*.lean}     \\
