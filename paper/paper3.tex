...
\begin{abstract}
	We establish a \emph{Reconfiguration Harmony Theorem} for asynchronous MPST. Under well-formed
	...
	composed-system conservation follow, including quantitative lift of the Computable Dynamics
	paper's Lyapunov framework with conservation for pure reconfiguration and descent or budget
...
(Toninho and Yoshida, 2017). In this paper, as in the first paper of this series, $\Coherent$ denotes
...
\begin{figure}[tbp]
	...
	\]
	\vspace{-8pt}
	\caption{Delegation footprint: rewrite only touched edges; preserve session-disjoint and
		unrelated edges by frame transport.}
...
Table~4 gives the typed guard clauses used to discharge $\mathsf{WF}_{\rho_{\mathsf{tr}}}$
...
\begin{theorem}[Reconfiguration Harmony]
	...
	For all configurations $C$ and reconfiguration operators $\rho \in \{\mathsf{link},
		\mathsf{delegate}\}$, if the Reconfiguration assumptions holds for $(C,\rho)$, then
	\[
		\ProjStep{C}{\rho}
		=
		\LocStep{C}{\rho}.
