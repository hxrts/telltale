...
\begin{definition}[$\Admissible(I)$]
	...
	\begin{enumerate}
		...
		\item Delegation adequacy: $I$ entails the local obligations required for safe delegation.
	\end{enumerate}
\end{definition}
...
\begin{proof}
	...
	By the footprint update discipline (Equation~\ref{eq:p3-delegation-footprint}),
	only edges in $F$ are rewritten, while non-footprint edges are frame-preserved.
	Using delegation-preservation/footprint lemmas, projection of the rewritten footprint equals the
	corresponding local delegated update, and frame preservation identifies unchanged projected components.
	Hence
	\[
		\mathsf{project}(C')
		=
...
\begin{theorem}[Safe Delegation Sufficiency]
	...
	For all configurations $C$ and delegation operations $\rho_{\mathsf{del}}$,
	\begin{equation}
		\begin{aligned}[t]
...
\begin{corollary}[Footprint Exactness Packaging]
	\label{cor:footprint}
	For all $C,\rho_{\mathsf{del}}$ satisfying the stated step and well-formedness side conditions,
	\begin{equation}
		\begin{aligned}[t]
...
\begin{corollary}[Compositional Exactness Under Non-Interference]
	...
	For all compositions $C_1 \otimes C_2$, if Assumption Block~\ref{assump:p3-envelope} holds for both components and non-interference premises hold across the composition boundary, then envelope exactness is preserved by composition:
	\begin{equation}
		\begin{aligned}
