...
Scope is restricted to asynchronous buffered semantics with explicit reconfiguration side conditions,
crash-stop fault assumptions where fault claims are stated, and declared envelope-admission profiles.
Claims outside those assumptions are not made.
...
Figure~\ref{fig:p3-commuting-square} records the commuting square for reconfiguration Harmony.
\begin{figure}[tbp]
	\centering
...
Table~\ref{tab:p3-model-assumptions} records the model assumptions for exact claims in this paper.
...
Table~\ref{tab:p3-notation} fixes notation used in this paper.
...
layer, and the higher-order graph-delta layer. Appendix~D provides the file-level mapping.
...
\begin{definition}[Envelope Predicates]
	...
	the envelope-admission obligations used in Theorems~F--G.
\end{definition}
...
\begin{table}[tbp]
	...
	\begin{tabularx}{\columnwidth}{@{}L{0.27\columnwidth}L{0.36\columnwidth}L{0.27\columnwidth}@{}}
		...
		\texttt{reachable(Coord)}   & crash-tolerance decider & path not cut         \\
...
\begin{enumerate}
	\item Static Harmony via \texttt{link}. Composition-level commutation and coherence
	      ...
	      \texttt{link\_harmony\_through\_link} and \texttt{link\_preserves\_coherent}.
...
congruence and quotient lift used by this section: \texttt{effectBisim\_congr\_link} and
\texttt{configEquiv\_iff\_effectBisim\_silent}.
...
Equation~\ref{eq:p3-admissible-order} summarizes the admissible-invariant order behind Theorem~D.
\begin{equation}
...
\begin{itemize}
	...
	\item \texttt{lyapunov\_conserved\_under\_balanced\_delegation}
\end{itemize}
	...
	\begin{enumerate}
		\item Pure reconfiguration (\texttt{link}/delegation):
		      \begin{itemize}
			      ...
			      \item show weighted measure $W$ is invariant under balanced delegation/rewiring (\texttt{lyapunov\_conserved\_under\_balanced\_delegation}).
		      \end{itemize}
...
\begin{itemize}
	\item \texttt{exchangeNormalization\_of\_e1BridgePremises}
	\item \texttt{spatialSubtypingMonotonicity\_of\_e1BridgePremises}
\end{itemize}
...
\begin{proof}[Proof sketch]
	...
	\begin{enumerate}
		\item Exchange normalization follows from bridge theorem \texttt{exchangeNormalization\_of\_e1BridgePremises}.
		\item Spatial monotonicity follows from \texttt{spatialSubtypingMonotonicity\_of\_e1BridgePremises}.
...
\begin{theorem}[Observational Adequacy and VM Adherence Modulo Envelope]
	...
	\begin{equation}
		\Obs(S_v) = \Obs(S_r).
	\end{equation}
	\textbf{(Adherence).} For all profiles $(P,\Pi)$, if the profile-side premises of Assumption Block~\ref{assump:p3-adequacy} hold and
	\begin{equation}
...
\begin{itemize}
	\item \texttt{protocolOutcome\_effectBisim\_of\_observationalEq}
	\item \texttt{compile\_refines\_observationalEq\_of\_effectBisim}
	\item \texttt{vmView\_effectBisim\_of\_VMCEquiv}
	\item \texttt{vmCEquiv\_of\_vmView\_effectBisim}
	\item \texttt{topology\_change\_preserves\_VMCEquiv\_via\_effectBisim}
\end{itemize}
...
\begin{proof}[Proof sketch]
	...
	\begin{enumerate}
		\item Protocol outcome bridge:
		      \begin{itemize}
			      \item connect observational equality and effect-bisim (\texttt{protocolOutcome\_effectBisim\_of\_observationalEq}).
		      \end{itemize}
		\item Compile/runtime refinement bridge:
		      \begin{itemize}
			      \item lift effect-bisim to compile-time observational refinement (\texttt{compile\_refines\_observationalEq\_of\_effectBisim}).
		      \end{itemize}
		\item VM view bridge:
		      \begin{itemize}
			      \item connect VM configuration equivalence and effect-bisim (\texttt{vmView\_effectBisim\_of\_VMCEquiv}, converse, and topology-preservation lemmas).
		      \end{itemize}
	\end{enumerate}
...
\begin{proposition}
	Under Assumption Block~\ref{assump:p3-reconfiguration}, Harmony (Theorem~B) and Relative Minimality (Theorem~D) imply that any admissible safe-reconfiguration invariant implies $\Coherent$ and therefore factors through the same canonical safety core.
\end{proposition}
...
\begin{proposition}
	Under Assumption Block~\ref{assump:p3-envelope}, Theorem~F and Theorem~G jointly characterize determinism modulo exchange/spatial normalization and identify strict boundary witnesses for inadmissible envelope extensions.
\end{proposition}
...
\begin{proposition}
	Under Assumption Block~\ref{assump:p3-adequacy}, Theorem~H plus Corollary~S1 yields principal-capability admission completeness with explicit rejection diagnostics when premise evidence is missing.
\end{proposition}

\begin{proof}[Proof sketch]
	Theorem~H yields adequacy and adherence under Assumption Block~\ref{assump:p3-adequacy}.  Corollary~S1 then refines this with principal-capability inference and admission completeness. Together they imply the proposition.
\end{proof}

\subsection{Byzantine Scope Boundary}

\begin{proposition}
	Under Assumption Block~\ref{assump:p3-byzantine}, the BZ family establishes exact safety characterization and converse counterexample packaging, but does not establish Byzantine liveness outside declared timing/fairness assumptions.
\end{proposition}

\begin{proof}[Proof sketch]
	Theorem~BZ and Corollary~BZ.1 provide exact Byzantine safety characterization and converse counterexample packaging under Assumption Block~\ref{assump:p3-byzantine}. Section~16 explicitly restricts scope away from weaker timing and fairness assumptions required for Byzantine liveness. Therefore the claimed boundary follows directly.
\end{proof}
...
\begin{table}[tbp]
	...
	\begin{tabularx}{\columnwidth}{@{}L{0.12\columnwidth}L{0.48\columnwidth}L{0.34\columnwidth}@{}}
		...
		\midrule
		B            & \nolinkurl{LinkingCore.lean}, \nolinkurl{Congruence.lean}   & \texttt{link\_harmony\_through\_link} \\
		A,C,D        & \nolinkurl{ErasureCharacterization.lean}                     & \texttt{coherent\_erasure\_stable}    \\
		E            & \nolinkurl{LinkingTheorems.lean}, \nolinkurl{Lyapunov.lean} & \texttt{flagship\_composed\_sys}      \\
		F,G          & \nolinkurl{EnvelopeCore/*.lean}                              & \texttt{consensusEnvelope\_exact}     \\
		H,S1         & \nolinkurl{Adequacy.lean}, \nolinkurl{TheoremPack/*.lean}   & \texttt{vm\_adequacy\_of\_trace}      \\
		BZ           & \nolinkurl{EnvelopeTheorems.lean}                            & \texttt{byzantineSafety\_exact}       \\
		\bottomrule
	\end{tabularx}
	\caption{Module map}
...
\begin{table}[!h]
	...
	\begin{tabularx}{\columnwidth}{@{}L{0.2\columnwidth}L{0.1\columnwidth}L{0.34\columnwidth}L{0.3\columnwidth}@{}}
		...
		Thm A          & \S5           & Core                  & \nolinkurl{ErasureCharacterization.lean}                \\
		Thm C          & \S6           & Reconfig.\ + coh.     & delegation lemmas               \\
		Cor.\ C.1      & \S6           & Reconfig.\ + thm C    & footprint equiv.                \\
		Thm D          & \S7           & Reconfig.\ + admis.   & \texttt{relative\_minimality} \\
