...
\begin{itemize}
	...
	\item \DeliveryModel parametricity,
...
layer, and the higher-order graph-delta layer. Appendix~\ref{app:p3-anchor-registry} provides the file-level mapping.
...
\begin{proof}[Proof sketch]
	...
	\begin{enumerate}
		...
		\item Quotient/effect boundary:
		      \begin{itemize}
			      \item use effect-bisim congruence under link and quotient bridge (\nolinkurl{effect_bisim_congr_link},\allowbreak\ \nolinkurl{config_equiv_iff_effect_bisim_silent}) to align protocol and observational views.
		      \end{itemize}
	\end{enumerate}
...
Mechanized anchors:
\begin{itemize}
	\item \nolinkurl{safe_delegation_local_necessity}
	\item \nolinkurl{safe_delegation_to_footprint}
	\item \nolinkurl{footprint_to_safe_delegation}
...
\begin{proof}[Proof sketch]
	For the forward direction, instantiate Theorem~\ref{thm:delegation} and project its obligations to the declared delegation footprint. For the reverse direction, apply \nolinkurl{footprint_to_safe_delegation} to reconstruct global safety from footprint obligations, then use strictness witnesses to show necessity of the side conditions. Combining both implications yields the stated equivalence.
\end{proof}
...
\begin{itemize}
	...
	\item \nolinkurl{vm_view_effect_bisim_of_vmc_equiv}
	\item \nolinkurl{vmc_equiv_of_vm_view_effect_bisim}
...
\begin{proof}[Proof sketch]
	...
	\begin{enumerate}
		...
		\item VM view bridge:
		      \begin{itemize}
			      \item connect VM configuration equivalence and effect-bisim (\nolinkurl{vm_view_effect_bisim_of_vmc_equiv},\allowbreak\ converse, and topology-preservation lemmas).
		      \end{itemize}
	\end{enumerate}
...
\begin{enumerate}
	\item Higher-order/graph-delta semantics:
	      \texttt{Protocol/Coherence/GraphDelta.lean},
	      \texttt{Protocol/Coherence/GraphDeltaHO.lean}.
	\item First-order collapse and equivalence/renaming support:
	      \texttt{Protocol/Coherence/ConfigEquiv.lean},
	      \texttt{Protocol/Coherence/ConfigEquivRenaming/*.lean},
	      \texttt{Protocol/Coherence/RoleSwap/*.lean}.
	\item Delegation microkernel:
	      \texttt{Protocol/Coherence/Delegation/Core/*.lean},
	      \texttt{Protocol/Coherence/Delegation/Preservation.lean}.
\end{enumerate}
...
\label{app:p3-anchor-registry}

Table~\ref{tab:p3-anchor-registry} maps theorem anchors to their source files.

\begin{table}[H]
	\centering
	\scriptsize
	\begin{tabularx}{\columnwidth}{@{}L{0.475\columnwidth}L{0.475\columnwidth}@{}}
		\toprule
		\thead{Anchor} & \thead{Path (relative to \texttt{lean/})} \\
		\midrule
		\texttt{link\_harmony\_through\_link} & \texttt{Protocol/Deployment/\allowbreak}\mbox{\texttt{LinkingTheorems.lean}} \\
		\texttt{coherent\_erasure\_stable} & \texttt{Protocol/Coherence/\allowbreak}\mbox{\texttt{ErasureCharacterization.lean}} \\
		\texttt{flagship\_composed\_system\_conservation} & \texttt{Protocol/Deployment/\allowbreak}\mbox{\texttt{LinkingTheorems.lean}} \\
		\texttt{consensus\_envelope\_exact\_of\_profile} & \texttt{Runtime/Proofs/Adapters/\allowbreak Distributed/}\mbox{\texttt{EnvelopeTheorems.lean}} \\
		\texttt{vm\_adequacy\_of\_trace\_consistency} & \texttt{Runtime/Adequacy/}\mbox{\texttt{Adequacy.lean}} \\
		\texttt{byzantine\_safety\_exact\_of\_profile} & \texttt{Runtime/Proofs/Adapters/\allowbreak Distributed/}\mbox{\texttt{EnvelopeTheorems.lean}} \\
		\texttt{canOperateUnderByzantineEnvelope} & \texttt{Runtime/Proofs/TheoremPack/}\mbox{\texttt{*.lean}} \\
		\texttt{effect\_bisim\_congr\_link} & \texttt{Runtime/Proofs/EffectBisim/\allowbreak}\mbox{\texttt{Congruence.lean}} \\
		\texttt{lyapunov\_conserved\_under\_balanced\_delegation} & \texttt{Runtime/Proofs/}\mbox{\texttt{Lyapunov.lean}} \\
		\bottomrule
	\end{tabularx}
	\caption{Anchor registry.}
	\label{tab:p3-anchor-registry}
\end{table}
...
\begin{enumerate}
	\item Build anchors listed in Table~\ref{tab:p3-anchor-registry}.
	\item Run \texttt{just escape} and \texttt{just verify-protocols}.
	\item Validate that Table~\ref{tab:p3-anchor-registry} anchor names resolve and no proof holes remain.
\end{enumerate}
...
\begin{table}[H]
	...
	\begin{tabularx}{\columnwidth}{@{}L{0.22\columnwidth}L{0.09\columnwidth}L{0.42\columnwidth}L{0.17\columnwidth}@{}}
		\toprule
		\thead{Claim}          & \thead{Sec.} & \thead{Assumptions} & \thead{Anchor} \\
		\midrule
		Thm.~\ref{thm:harmony}            & \S\ref{sec:theorem-b} & Assump.~\ref{assump:p3-reconfiguration} & \texttt{link\_harmony*} \\
		Thm.~\ref{thm:erasure}            & \S\ref{sec:theorem-a} & Assump.~\ref{assump:p3-core}            & \texttt{coherent\_*} \\
		Thm.~\ref{thm:delegation}         & \S\ref{sec:theorem-c} & Assump.~\ref{assump:p3-reconfiguration} & \texttt{coherent\_*} \\
		Cor.~\ref{cor:footprint}          & \S\ref{sec:theorem-c} & Thm.~\ref{thm:delegation} + side conds  & \texttt{coherent\_*} \\
		Thm.~\ref{thm:minimality}         & \S\ref{sec:theorem-d} & admissibility clauses + core model       & \texttt{coherent\_*} \\
		Thm.~\ref{thm:conservation}       & \S\ref{sec:theorem-e} & Assump.~\ref{assump:p3-reconfiguration} + quant.\ profile & \texttt{flagship\_*} \\
		Cor.~\ref{cor:compositional}      & \S\ref{sec:theorem-e} & non-interference + envelope exactness    & \S\ref{sec:theorem-e} \\
		Thm.~\ref{thm:envelope}           & \S\ref{sec:theorem-f} & Assump.~\ref{assump:p3-envelope}         & \texttt{consensus*} \\
		Cor.~\ref{cor:transportability}   & \S\ref{sec:theorem-f} & Assump.~\ref{assump:p3-envelope}         & \texttt{consensus*} \\
		Cor.~\ref{cor:boundary}           & \S\ref{sec:theorem-f} & Assump.~\ref{assump:p3-envelope}         & \texttt{consensus*} \\
		Cor.~\ref{cor:embedding}          & \S\ref{sec:theorem-f} & Assump.~\ref{assump:p3-envelope}         & \texttt{consensus*} \\
		Thm.~\ref{thm:normalization}      & \S\ref{sec:theorem-g} & Assump.~\ref{assump:p3-envelope} + exch./spatial premises & \texttt{consensus*} \\
		Thm.~\ref{thm:adequacy}           & \S\ref{sec:theorem-h} & Assump.~\ref{assump:p3-adequacy}         & \texttt{vm\_adequacy*} \\
		Prop.~\ref{prop:capability}       & \S\ref{sec:theorem-h} & Assump.~\ref{assump:p3-adequacy}         & \texttt{canOperate*} \\
		Cor.~\ref{cor:principal}          & \S\ref{sec:theorem-h} & adequacy + admission premises            & \texttt{canOperate*} \\
		Thm.~\ref{thm:byzantine}          & \S\ref{sec:theorem-h} & Assump.~\ref{assump:p3-byzantine}        & \texttt{byzantine*} \\
		Cor.~\ref{cor:byzantine-converse} & \S\ref{sec:theorem-h} & Thm.~\ref{thm:byzantine} + dropped class & \texttt{byzantine*} \\
		Prop.~\ref{prop:byzantine-vm}     & \S\ref{sec:theorem-h} & adequacy + Byzantine profile + capability bits & \texttt{canOperate*} \\
