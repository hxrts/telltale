...
\begin{figure}[H]
	...
	\end{tikzpicture}
	\caption{Dual-track trace view: reconfiguration steps and projected local steps align under Harmony, while admitted runtime traces remain inside the exact envelope. This figure intentionally scopes to $\rho\in\{\mathsf{link},\mathsf{delegate}\}$ (transition choreography is handled later), and $\ell_i$ denotes the projected local step corresponding to $\rho_i$.}
...
\begin{proof}[Proof sketch]
	By definition of capability extraction, each advertised bit is accepted only if the corresponding theorem-pack entry is present and typechecks against the same profile premises used by admission. The runtime gate \texttt{canOperateUnderByzantineEnvelope} checks required capability evidence. Soundness of profile extraction then yields a witness object $a_b$ for each advertised bit. Therefore advertised capabilities are justified by theorem-pack evidence under the Theorem~\ref{thm:adequacy} premise profile.
\end{proof}
...
\begin{enumerate}
	\item Build anchors listed in Table~\ref{tab:p3-index}.
	\item Run \texttt{just escape} and \texttt{just verify-protocols}.
	\item Validate that anchor names resolve and no proof holes remain.
\end{enumerate}
...
\begin{table}[H]
	\centering
	\footnotesize
	\begin{tabularx}{\columnwidth}{@{}L{0.53\columnwidth}L{0.42\columnwidth}@{}}
		\toprule
		\thead{Claim} & \thead{Assumption Block(s)} \\
		\midrule
		Theorems~\ref{thm:harmony}--\ref{thm:conservation}, Corollary~\ref{cor:footprint} & \ref{assump:p3-reconfiguration} \\
		Theorems~\ref{thm:envelope}, \ref{thm:normalization}; Corollaries~\ref{cor:compositional}--\ref{cor:embedding} & \ref{assump:p3-envelope} \\
		Theorem~\ref{thm:adequacy}, Proposition~\ref{prop:capability}, Corollary~\ref{cor:principal} & \ref{assump:p3-adequacy} \\
		Theorem~\ref{thm:byzantine}, Corollary~\ref{cor:byzantine-converse}, Proposition~\ref{prop:byzantine-vm} & \ref{assump:p3-byzantine}, plus \ref{assump:p3-adequacy} for VM-adherence claims \\
		\bottomrule
	\end{tabularx}
...
Table~\ref{tab:p3-index} maps paper claims to their Lean anchors and source files. Anchors marked with --- are infrastructure lemmas used in proofs but not tied to a single claim.
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{L{0.29\textwidth}L{0.37\textwidth}L{0.06\textwidth}>{\raggedright\arraybackslash}X}
		...
		\rowcolor{gray!8}
		\texttt{link\_harmony\_\allowbreak through\_link}                                            & \texttt{Protocol/\allowbreak Deployment/\allowbreak}\mbox{\texttt{LinkingTheorems.lean}}                                          & \S\ref{sec:theorem-b} & Thm.~\ref{thm:harmony}                                                               \\
		\texttt{link\_preserves\_\allowbreak coherent}                                               & \texttt{Protocol/\allowbreak Deployment/\allowbreak}\mbox{\texttt{LinkingCore.lean}}                                              & \S\ref{sec:theorem-b} & Thm.~\ref{thm:harmony}                                                               \\
		\rowcolor{gray!8}
		...
		\rowcolor{gray!8}
		\texttt{coherent\_erasure\_\allowbreak stable}                                               & \texttt{Protocol/\allowbreak Coherence/\allowbreak}\mbox{\texttt{ErasureCharacterization.lean}}                                   & \S\ref{sec:theorem-a} & Thm.~\ref{thm:erasure}                                                               \\
		...
		\rowcolor{gray!8}
			...
			\texttt{footprint\_to\_\allowbreak safe\_delegation}                                         & \texttt{Protocol/\allowbreak Coherence/\allowbreak}\mbox{\texttt{ErasureCharacterization.lean}}                                   & \S\ref{sec:theorem-c} & Cor.~\ref{cor:footprint}                                                             \\
			\rowcolor{gray!8}
			\texttt{safe\_delegation\_\allowbreak iff\_footprint}                                        & \texttt{Protocol/\allowbreak Coherence/\allowbreak}\mbox{\texttt{ErasureCharacterization.lean}}                                   & \S\ref{sec:theorem-c} & Cor.~\ref{cor:footprint}                                                             \\
		\texttt{relative\_minimality}                                                                & \texttt{Protocol/\allowbreak Coherence/\allowbreak}\mbox{\texttt{ErasureCharacterization.lean}}                                   & \S\ref{sec:theorem-d} & Thm.~\ref{thm:minimality}                                                            \\
		\rowcolor{gray!8}
		\texttt{flagship\_composed\_\allowbreak system\_conservation}                                & \texttt{Protocol/\allowbreak Deployment/\allowbreak}\mbox{\texttt{LinkingTheorems.lean}}                                          & \S\ref{sec:theorem-e} & Thm.~\ref{thm:conservation}                                                          \\
		...
		\rowcolor{gray!8}
		\texttt{consensus\_envelope\_\allowbreak exact\_of\_profile}                                 & \texttt{Runtime/\allowbreak Proofs/\allowbreak Adapters/\allowbreak Distributed/\allowbreak}\mbox{\texttt{EnvelopeTheorems.lean}} & \S\ref{sec:theorem-f} & Thm.~\ref{thm:envelope}, Cors.~\ref{cor:transportability}--\ref{cor:embedding}       \\
		...
		\rowcolor{gray!8}
		\texttt{exchange\_normalization\_\allowbreak of\_e1\_bridge\_premises}                       & \texttt{Runtime/\allowbreak Adequacy/\allowbreak EnvelopeCore/\allowbreak}\mbox{\texttt{ReconfigurationBridge.lean}}              & \S\ref{sec:theorem-g} & Thm.~\ref{thm:normalization}                                                         \\
		\texttt{spatial\_subtyping\_\allowbreak monotonicity\_of\_e1\_\allowbreak bridge\_premises}  & \texttt{Runtime/\allowbreak Adequacy/\allowbreak EnvelopeCore/\allowbreak}\mbox{\texttt{ReconfigurationBridge.lean}}              & \S\ref{sec:theorem-g} & Thm.~\ref{thm:normalization}                                                         \\
		\rowcolor{gray!8}
		\texttt{vm\_adequacy\_of\_\allowbreak trace\_consistency}                                    & \texttt{Runtime/\allowbreak Adequacy/\allowbreak}\mbox{\texttt{Adequacy.lean}}                                                    & \S\ref{sec:theorem-h} & Thm.~\ref{thm:adequacy}                                                              \\
		...
		\rowcolor{gray!8}
		\texttt{byzantine\_safety\_\allowbreak exact\_of\_profile}                                   & \texttt{Runtime/\allowbreak Proofs/\allowbreak Adapters/\allowbreak Distributed/\allowbreak}\mbox{\texttt{EnvelopeTheorems.lean}} & \S\ref{sec:theorem-h} & Thm.~\ref{thm:byzantine}, Cor.~\ref{cor:byzantine-converse}                          \\
		\midrule
		\multicolumn{4}{@{}l}{\textit{Runtime Gates}}                                                                                                                                                                                                                                                                                                   \\
		\rowcolor{gray!8}
		\texttt{canOperateUnder\allowbreak ByzantineEnvelope}                                        & \texttt{Runtime/\allowbreak Proofs/\allowbreak TheoremPack/\allowbreak}\mbox{\texttt{API.lean}}                                   & \S\ref{sec:theorem-h} & Prop.~\ref{prop:capability}, Cor.~\ref{cor:principal}, Prop.~\ref{prop:byzantine-vm} \\
		\bottomrule
	\end{tabularx}
