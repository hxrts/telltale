...
Scope is confined to asynchronous buffered semantics with explicit reconfiguration side conditions,
...
{\scriptsize
		\begin{equation}
			\dfrac{
				\mathsf{WF}_{\rho_{\mathsf{tr}}}(C_2);\ \Coherent(C_2);\ \text{transition-profile premises}
			}{
				C_2 \to^\ast C_3;\ \text{budget-certified descent}
			}
		\end{equation}
}
...
\subsection{Artifact Path Registry}
...
Expected outcome: Full theorem-dependency compilation (B through H, S1, BZ) succeeds, profile extraction and admission modules compile, and bridge and adequacy modules compile under the pinned toolchain.
...
\begin{table}[H]
	\centering
	\footnotesize
	\begin{tabularx}{\columnwidth}{@{}L{0.23\columnwidth}L{0.09\columnwidth}L{0.33\columnwidth}L{0.29\columnwidth}@{}}
		\toprule
		\thead{Claim}          & \thead{Sec.} & \thead{Assumptions}                   & \thead{Location}        \\
		\midrule
		Thm B                  & \S4          & Reconfig.                             & linking + congruence    \\
		Thm A                  & \S5          & Core                                  & \pathref{P3-A10}        \\
		Thm C                  & \S6          & Reconfig.\ + coh.                     & delegation lemmas       \\
		Cor.\ C.1              & \S6          & Reconfig.\ + thm C                    & footprint equiv.        \\
		Thm D                  & \S7          & Reconfig.\ + admis.                   & minimality layer        \\
		Thm E                  & \S8          & Reconfig.\ + quant.                   & conservation            \\
		Cor.\ S2               & \S8          & Reconfig.\ + non-int.                 & transport schema        \\
		Thm F                  & \S9          & Envelope                              & envelope-core           \\
		Cor.\ Finite-Erasure   & \S9          & Envelope (\ref{assump:p3-envelope})   & Appendix~A/C witnesses  \\
		Cor.\ Boundary Witness & \S9          & Envelope (\ref{assump:p3-envelope})   & boundary witness family \\
		Cor.\ Embedding        & \S9          & Envelope (\ref{assump:p3-envelope})   & embedding bridge        \\
		Thm G                  & \S10         & Envelope + exch.                      & normalization           \\
		Thm H                  & \S11         & Adequacy                              & adequacy bridge         \\
		Prop.\ Capability-Bit  & \S11         & Adequacy (\ref{assump:p3-adequacy})   & capability extraction   \\
		Cor.\ S1               & \S11         & Adequacy + admis.                     & profile extract.        \\
		Thm BZ                 & \S11         & Byzantine                             & \pathref{P3-A16}        \\
		Cor.\ BZ.1             & \S11         & Byzantine + drop                      & Section 11              \\
		Prop.\ BZ.2            & \S11         & Adequacy + Byz.                       & conformance             \\
		Thm.\ Tail-Bound       & \S13         & Transport (\ref{assump:p3-transport}) & Section~13 transport    \\
		Thm.\ Mixing-Rate      & \S13         & Transport (\ref{assump:p3-transport}) & Section~13 transport    \\
		\bottomrule
	\end{tabularx}
