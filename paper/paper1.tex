...
The split operator is implemented by \nolinkurl{edge_case_split}. Rule-level preservation lemmas are grouped by send, recv, select, and branch cases. Table~\ref{tab:p1-anchor-registry} gives the anchor-to-file mapping, and \cref{app:chains} records theorem-to-lemma chains.
...
\end{enumerate}
	...
	\texttt{VMBridgePremises},
	\nolinkurl{handler_obligation_local},
	\nolinkurl{handler_fragment_typing_of_premises},
...
\textbf{Lemma chain.}
\begin{enumerate}
	\item Expand \ByzIfaceWF as definitional \Coherent.
	\item On each active edge, extract sender witness and \Consume success.
\end{enumerate}
...
\label{app:p1-anchor-registry}

Table~\ref{tab:p1-anchor-registry} maps theorem anchors to their source files.
...
\begin{table}[H]
	...
	\begin{tabularx}{\columnwidth}{@{}L{0.475\columnwidth}L{0.475\columnwidth}@{}}
		...
		\midrule
		\texttt{Consume} & \texttt{Protocol/Coherence/}\mbox{\texttt{Consume.lean}} \\
		\texttt{consume\_append} & \texttt{Protocol/Coherence/}\mbox{\texttt{Consume.lean}} \\
		\texttt{consume\_cons} & \texttt{Protocol/Coherence/}\mbox{\texttt{Consume.lean}} \\
		\texttt{consume\_mono} & \texttt{Protocol/Coherence/\allowbreak}\mbox{\texttt{SubtypeReplacement*.lean}} \\
		\texttt{EdgeCoherent} & \texttt{Protocol/Coherence/\allowbreak}\mbox{\texttt{EdgeCoherenceCore.lean}} \\
		\texttt{Coherent} & \texttt{Protocol/Coherence/\allowbreak}\mbox{\texttt{EdgeCoherenceCore.lean}} \\
		\texttt{edge\_case\_split} & \texttt{Protocol/Coherence/}\mbox{\texttt{Unified.lean}} \\
		\texttt{coherent\_send\_preserved} & \texttt{Protocol/Coherence/\allowbreak}\mbox{\texttt{Preservation*.lean}} \\
		\texttt{coherent\_recv\_preserved} & \texttt{Protocol/Coherence/\allowbreak}\mbox{\texttt{PreservationRecv.lean}} \\
		\texttt{coherent\_select\_preserved} & \texttt{Protocol/Coherence/\allowbreak}\mbox{\texttt{SelectPreservation.lean}} \\
		\texttt{coherent\_branch\_preserved} & \texttt{Protocol/Coherence/\allowbreak}\mbox{\texttt{SelectPreservation.lean}} \\
		\texttt{edge\_coherent\_type\_replacement} & \texttt{Protocol/Coherence/\allowbreak}\mbox{\texttt{SubtypeReplacement*.lean}} \\
		\texttt{coherent\_type\_replacement} & \texttt{Protocol/Coherence/\allowbreak}\mbox{\texttt{SubtypeReplacement*.lean}} \\
		\texttt{progress\_conditions\_type\_replacement} & \texttt{Protocol/Coherence/\allowbreak}\mbox{\texttt{SubtypeReplacement*.lean}} \\
		\texttt{fifo\_delivery\_laws} & \texttt{Protocol/}\mbox{\texttt{DeliveryModel.lean}} \\
		\texttt{causal\_delivery\_laws} & \texttt{Protocol/}\mbox{\texttt{DeliveryModel.lean}} \\
		\texttt{lossy\_delivery\_laws} & \texttt{Protocol/}\mbox{\texttt{DeliveryModel.lean}} \\
		\texttt{monitor\_sound} & \texttt{Runtime/VM/Runtime/}\mbox{\texttt{Monitor.lean}} \\
		\texttt{unified\_monitor\_preserves} & \texttt{Runtime/VM/Runtime/}\mbox{\texttt{Monitor.lean}} \\
		\texttt{VMBridgePremises} & \texttt{Runtime/Proofs/VM/\allowbreak}\mbox{\texttt{BridgeStrengthening.lean}} \\
		\texttt{handler\_obligation\_local} & \texttt{Runtime/Proofs/VM/\allowbreak}\mbox{\texttt{BridgeStrengthening.lean}} \\
		\texttt{vm\_bridge\_soundness\_composed} & \texttt{Runtime/Proofs/VM/\allowbreak}\mbox{\texttt{BridgeStrengthening.lean}} \\
		\texttt{effect\_bisim\_implies\_observational\_equivalence} & \texttt{Runtime/Proofs/EffectBisim/}\mbox{\texttt{Bridge.lean}} \\
		\texttt{config\_equiv\_iff\_effect\_bisim\_silent} & \texttt{Runtime/Proofs/EffectBisim/\allowbreak}\mbox{\texttt{ConfigEquivBridge.lean}} \\
		\texttt{ByzIfaceWF} & \texttt{Runtime/Proofs/Adapters/Distributed/\allowbreak}\mbox{\texttt{EnvelopeTheorems.lean}} \\
		\texttt{byz\_iface\_wf\_iff\_coherent} & \texttt{Runtime/Proofs/Adapters/Distributed/\allowbreak}\mbox{\texttt{EnvelopeTheorems.lean}} \\
		\texttt{canOperateUnderByzantineEnvelope} & \texttt{Runtime/Proofs/TheoremPack/}\mbox{\texttt{API.lean}} \\
		\bottomrule
	\end{tabularx}
	\caption{Anchor registry.}
	\label{tab:p1-anchor-registry}
\end{table}
...
\begin{center}
	...
	\begin{tabularx}{\columnwidth}{@{}L{0.2\columnwidth}L{0.08\columnwidth}L{0.42\columnwidth}L{0.20\columnwidth}@{}}
		\toprule
		\thead{Claim} & \thead{Sec.} & \thead{Assumption} & \thead{Anchor} \\
		\midrule
			Theorem 4.1    & \S4           & Core model premises (\ref{assump:p1-core})               & \texttt{coherent\_\*\_preserved}  \\
			Theorem 6.1    & \S6           & Replacement compatibility (\ref{assump:p1-replacement})  & \texttt{consume\_mono}  \\
			Prop.\ 8.1     & \S8           & Core model + bridge obligations                          & \texttt{vm\_bridge\_*}  \\
			Thm.\ BZ-1     & \S8           & Byzantine interface premises (\ref{assump:p1-byzantine}) & \ByzIfaceWF  \\
			Cor.\ BZ-1.1   & \S8           & Byzantine interface + witnesses                          & derived           \\
			Prop.\ BZ-1.2  & \S8           & Byzantine interface + gate                               & \texttt{canOperate*}  \\
			\bottomrule
		\end{tabularx}
	}
