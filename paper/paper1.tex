...
global-first organization (Honda et al., 1998; Scalas and Yoshida, 2019), while
...
\texttt{Consume\_mono}, which we lift to global Coherence and progress-condition preservation under
compatibility premises.

This is the first publication in a three-part series on asynchronous buffered semantics with
\texttt{DeliveryModel} parameterization. This paper precisely defines the invariant kernel.
...
\begin{enumerate}
	...
	\item A subtype-replacement theorem stack from \texttt{Consume\_mono} that preserves edge Coherence, global Coherence, and progress-side conditions under receive-compatibility premises.
...
\begin{table}[tbp]
	\centering
	\small
	\begin{tabularx}{\columnwidth}{@{}L{0.4\columnwidth}L{0.55\columnwidth}@{}}
		\toprule
...
The split operator is implemented by \texttt{edge\_case\_split}. Rule-level preservation lemmas are grouped by send, recv, select, and branch cases. Table~\ref{tab:p1-claim-artifact-map} gives the primary module map, and \cref{app:chains} records theorem-to-lemma chains with exact Lean anchors.
...
The case partition used by \texttt{edge\_case\_split} is:
\begin{equation}
	\begin{array}{c}
...
\begin{table}[tbp]
	...
	\begin{tabularx}{\columnwidth}{@{}L{0.27\columnwidth}L{0.36\columnwidth}L{0.27\columnwidth}@{}}
		...
		\midrule
		Definitions       & \nolinkurl{Consume.lean}, \nolinkurl{EdgeCoherence-Core.lean} & \texttt{Consume}, \texttt{EdgeCoherent} \\
		Consume kernel    & \nolinkurl{Consume.lean}                                      & \texttt{Consume\_append}                \\
		Three-way split   & \nolinkurl{Unified.lean}                                      & \texttt{edge\_case\_split}              \\
		Preservation      & \nolinkurl{Preservation*.lean}                                & \texttt{Coherent\_*\_preserved}         \\
		Delivery variants & \nolinkurl{DeliveryModel.lean}                                & \texttt{*Delivery\_laws}                \\
		Subtype repl.     & \nolinkurl{SubtypeReplacement*.lean}                          & \texttt{Consume\_mono}                  \\
		\bottomrule
	\end{tabularx}
...
\begin{proposition}[Bridge Soundness]
	For any selected handler $h$, if choreography obligations for $h$ are discharged, \texttt{EffectSpec.handlerType} holds for $h$, \texttt{monitor\_sound} and \texttt{unified\_monitor\_preserves} hold for the active monitor, and the configuration-equivalence/effect-bisimulation bridge assumptions hold, then VM-side typing obligations generated from $h$ are satisfied in the corresponding runtime instruction layer and transport to the observational interface.
\end{proposition}

\begin{proof}[Proof sketch]
	Build the bundled bridge premises \texttt{VMBridgePremises} from \texttt{monitor\_sound}, \texttt{unified\_monitor\_preserves}, and the \texttt{WellTypedInstr.wt\_invoke} handler-typing witness. Lift handler typing from local obligations to short instruction fragments and then to VM-step form through \texttt{handler\_obligation\_local}, \texttt{handler\_fragment\_typing\_of\_premises}, and \texttt{handler\_vm\_step\_typing}. Compose with \texttt{configEquiv\_iff\_effectBisim\_silent} and \texttt{effectBisim\_implies\_observationalEquivalence} via \texttt{vm\_bridge\_soundness\_composed}. Therefore the runtime typing obligations and their observational transport both follow under the stated premises.
\end{proof}
...
\begin{itemize}
	...
	\item \nolinkurl{lean/Runtime/Proofs/VM/BridgeStrengthening.lean}: \texttt{VMBridgePremises}, \texttt{handler\_obligation\_local}, \texttt{handler\_fragment\_typing\_of\_premises}, \texttt{handler\_vm\_step\_typing}, \texttt{vm\_bridge\_soundness\_composed}
...
\begin{center}
	...
	\begin{tabularx}{\columnwidth}{@{}L{0.2\columnwidth}L{0.08\columnwidth}L{0.3\columnwidth}L{0.32\columnwidth}@{}}
		...
		\midrule
		Theorem 1      & \S4           & Core model premises (\ref{assump:p1-core})               & \nolinkurl{Preservation*.lean} \\
		Theorem 2      & \S6           & Replacement compatibility (\ref{assump:p1-replacement})  & \nolinkurl{SubtypeRepl*.lean}  \\
		Prop.\ 1       & \S8           & Core model + bridge obligations                          & runtime bridge                 \\
		Thm.\ BZ-1     & \S8           & Byzantine interface premises (\ref{assump:p1-byzantine}) & \nolinkurl{EnvelopeThms.lean}  \\
		Cor.\ BZ-1.1   & \S8           & Byzantine interface + witnesses                          & derived                        \\
		Prop.\ BZ-1.2  & \S8           & Byzantine interface + gate                               & \nolinkurl{TheoremPack/*.lean} \\
		\bottomrule
	\end{tabularx}
