...
\begin{figure}[H]
	...
	\begin{tikzpicture}[figfont, x=1mm, y=1mm]
		...
		\draw[rounded corners=6pt] (-38,-14) rectangle (38,18);
		\node at (0,15) {\textbf{Operational state space}};

		% Coherent invariant region
		\draw[rounded corners=6pt, fill=gray!10] (-35,-11) rectangle (35,12);
		\node at (0,9) {\textbf{\textsf{Coherent} invariant region}};

		% Typed-step trajectory
		\fill (-24,0) circle (1.4pt);
		\fill (-10,0) circle (1.4pt);
		\draw[figarrowstrong] (-24,0) -- (-10,0);
		\node[anchor=south] at (-17,1) {typed step};

		% Subtype-replacement trajectory
		\fill (9,0) circle (1.4pt);
		\fill (23,0) circle (1.4pt);
		\draw[figarrowstrong] (9,0) -- (23,0);
		\node[anchor=south] at (16,1) {subtype replacement};

		% Symbolic statement
		\node[align=center] at (0,-6)
		{$\Coherent \xrightarrow{\text{\tiny \hphantom{en}step\hphantom{en}}} \Coherent
				\quad
				\Coherent \xrightarrow{\!\text{\tiny replacement}\!} \Coherent$};
	\end{tikzpicture}
