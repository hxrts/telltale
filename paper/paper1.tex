...
\begin{definition}[consumeOne]
	...
	\[
		\consumeOne(r,T,L)=
		\begin{cases}
			\mathsf{some}(L') & \text{if } L=\mathsf{recv}(r,T,L'), \\
			\mathsf{none}     & \text{if } L\in\{\mathsf{send},\mathsf{select},\mathsf{branch},\mathsf{end},\mathsf{var}\}, \\
			\mathsf{none}     & \text{if } L=\mu L_0 \text{ at this layer.}
		\end{cases}
	\]
...
\begin{lemma}[Three-Way Edge-Split Exhaustiveness]
	...
	\[
		e=u,\qquad
		e\neq u\land \EdgeShares(e,ep_{\mathrm{step}}),\qquad
		e\neq u\land \neg\EdgeShares(e,ep_{\mathrm{step}}).
	\]
\end{lemma}
...
The square states the commuting condition used across these three papers.
...
Complexity note. Under constant-time constructor tests in \consumeOne, one
\Consume check on edge trace $ts_e$ costs $O(|ts_e|)$. Therefore one full
active-edge Coherence check costs
\[
	O\!\Big(\sum_{e\in E_{\mathrm{active}}}|ts_e|\Big),
\]
which is $O(|E_{\mathrm{active}}|\cdot m_{\max})$ for
$m_{\max}:=\max_e |ts_e|$. The dominant runtime term is buffered-trace scan
length, not endpoint-map lookup.
...
\texttt{EffectSpec.handlerType} records typed effect obligations at the VM boundary.
The VM instruction layer used in this paper has \texttt{send}, \texttt{recv}, \texttt{select},
\texttt{branch}, and \texttt{invoke} forms. \texttt{WellTypedInstr} maps each instruction form to a
session-side obligation. Send and select forms use sender-side continuation obligations. Receive and
branch forms use \Consume-side alignment obligations. Invoke form lifts handler obligations to instruction fragments.
Write this mapping as
\begin{equation}
	\begin{aligned}[t]
		\mathsf{WTObl}(\texttt{send})   & = \mathsf{SendCont},         \\
		\mathsf{WTObl}(\texttt{recv})   & = \mathsf{RecvConsumeCont},  \\
		\mathsf{WTObl}(\texttt{select}) & = \mathsf{SelectBranchCont}, \\
		\mathsf{WTObl}(\texttt{branch}) & = \mathsf{BranchCaseCont},   \\
		\mathsf{WTObl}(\texttt{invoke}) & = \mathsf{InvokeLift}.
	\end{aligned}
\end{equation}
where each right-hand side names the corresponding session obligation family in \texttt{WellTypedInstr}.
...
\begin{table}[H]
	\centering
	\scriptsize
	\begin{tabularx}{\columnwidth}{@{}L{0.24\columnwidth}L{0.71\columnwidth}@{}}
		\toprule
		\thead{Instruction Form} & \thead{Session-Side Typing Obligation} \\
		\midrule
		\texttt{send} & continuation typing with enqueue witness \\
		\texttt{recv} & \Consume-alignment witness with continuation typing \\
		\texttt{select}, \texttt{branch} & label agreement with branch-continuation typing \\
		\texttt{invoke} & handler-obligation lift to typed VM fragment \\
		\bottomrule
	\end{tabularx}
...
\begin{corollary}[Dropped-Assumption Witness Interface (BZ-1.1)]
	\label{cor:bz1-1}
	Under Assumption Block~\ref{assump:p1-byzantine}, let $\mathcal{A}$ be any assumption class named in \Hbyz. If a later construction provides a witness violating \ByzSafe when $\mathcal{A}$ is dropped, then that witness can be expressed against the same active-edge Coherence interface used in this paper.
\end{corollary}

\begin{proposition}[Capability-Gated VM Interface (BZ-1.2)]
	\label{prop:bz1-2}
	Under Assumption Block~\ref{assump:p1-byzantine}, if a runtime profile includes Byzantine characterization and envelope-adherence capability evidence, then safety-visible runtime obligations are interpreted through \Eqsafe modulo \Ebyz, and profile admission is blocked when required evidence is absent.
\end{proposition}
...
\texttt{Protocol/Coherence/*.lean} for the core invariant and
\texttt{Runtime/Proofs/VM/BridgeStrengthening.lean} with
\texttt{Runtime/VM/Runtime/Monitor.lean} for VM bridge obligations.
...
Lossy instance. Delivered traces are order-preserving subsequences of sent traces
on each edge. This instance is provided by \nolinkurl{lossy_delivery_laws}.

Safety scope under message loss. Coherence-preservation claims are stated on the
post-step delayed-trace environment $D'$. If a delivery instance drops a message,
...
\begin{table}[H]
	...
	\begin{tabularx}{\columnwidth}{@{}L{0.34\columnwidth}L{0.61\columnwidth}@{}}
		...
		Pinned commit & \texttt{c3a6eec66380d8fe}\allowbreak\texttt{d287219af4f1baea}\allowbreak\texttt{d669ebb1} \\
		Archival DOI & \texttt{DOI pending artifact snapshot minting (replace before submission)} \\
		Lean source statistics & 1299 files, 205752 LOC, axioms: 0, unresolved proof holes (\texttt{sorry}): 0 \\
		\bottomrule
	\end{tabularx}
...
\subsection{Theorem-to-Assumption Map}

\begin{table}[H]
	\centering
	\footnotesize
	\begin{tabularx}{\columnwidth}{@{}L{0.50\columnwidth}L{0.45\columnwidth}@{}}
		\toprule
		\thead{Claim} & \thead{Assumption Block(s)} \\
		\midrule
		Theorem~\ref{thm:coherence-preservation}, Theorem~\ref{thm:session-evolution}, Proposition~\ref{prop:bridge-soundness} & \ref{assump:p1-core}, \ref{assump:p1-replacement} as cited per claim \\
		Theorem~\ref{thm:bz1}, Corollary~\ref{cor:bz1-1}, Proposition~\ref{prop:bz1-2} & \ref{assump:p1-byzantine} \\
		Fairness-dependent claims & none at this paper theorem layer; fairness is consumed in later-series quantitative lifts \\
		\bottomrule
	\end{tabularx}
	\caption{Main claims and their assumption blocks.}
\end{table}
