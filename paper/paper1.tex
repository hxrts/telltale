...
\texttt{DeliveryModel} parameterization. This paper precisely defines the invariant kernel. The second paper
develops quantitative bounds and decision procedures on that kernel. And the third paper proves
reconfiguration commutation and envelope exactness.
...
This paper does not introduce coherence as a new MPST notion. It refines established coherence with an operational invariant $\Coherent(G,D)$ over local environments and buffered traces. The novelty is an asynchronous preservation architecture that discharges obligations edge-locally and avoids per-step global re-derivation.
