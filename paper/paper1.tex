...
Termination argument. Every recursive call consumes one trace head, so
$|ts|$ strictly decreases. The recursion stops at $[]$.

Recursive local types. If $L$ has constructor $\mu L_0$, then $\consumeOne$
matches on one-step unfolding before checking the current message constructor.
Unfolding does not create extra $\Consume$ recursion depth because recursive
calls are still indexed only by remaining trace length.
