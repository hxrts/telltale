...
DETERMINISM_RUNTIME_SCOPE=(
  "${RUST_DIR}/machine/src"
  "${RUST_DIR}/simulator/src"
)

DETERMINISM_TEST_SCOPE=(
  "${RUST_DIR}/machine/tests"
  "${RUST_DIR}/simulator/tests"
)

VM_KERNEL_SCOPE=(
  "${RUST_DIR}/machine/src/kernel.rs"
  "${RUST_DIR}/machine/src/engine"
  "${RUST_DIR}/machine/src/threaded"
  "${RUST_DIR}/machine/src/scheduler"
  "${RUST_DIR}/machine/src/session"
  "${RUST_DIR}/machine/src/coroutine.rs"
  "${RUST_DIR}/machine/src/exec"
)
...
machine_core_allow_hits="$(rg -n --pcre2 '^[[:space:]]*#[[:space:]]*\\[[[:space:]]*allow\\([[:space:]]*clippy::[^)]+\\)[[:space:]]*\\]' "${RUST_DIR}/machine/src" -g '*.rs' || true)"
...
float_type_hits="$(filter_out_paths "${float_type_hits}" '/machine/tests/helpers/')"
