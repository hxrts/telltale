...
use rumpsteak_aura_choreography::{
    ast::{
        ...
        RoleValidationError,
        Choreography, Protocol, MessageType, LocalType
    },
    compiler::{
...
fn test_dynamic_role_creation() {
    ...
    let worker_symbolic = Role::with_param(format_ident!("Worker"), RoleParam::Symbolic("N".to_string()));
...
fn test_role_validation_overflow_protection() {
    ...
    assert!(matches!(result, Err(RoleValidationError::CountOverflow { .. })));
    ...
    assert!(matches!(result, Err(RoleValidationError::IndexOverflow { .. })));
...
fn test_role_range_validation() {
    ...
    assert!(matches!(result, Err(RoleValidationError::InvalidRange { .. })));
    ...
    assert!(matches!(result, Err(RoleValidationError::RangeSizeOverflow { .. })));
}
...
fn test_dynamic_role_code_generation() {
    ...
    let coordinator = Role::new(format_ident!("Coordinator"));
    let signers = Role::with_param(format_ident!("Signers"), RoleParam::Runtime);
    let workers = Role::with_param(format_ident!("Workers"), RoleParam::Symbolic("N".to_string()));
...
fn test_complex_dynamic_protocol() {
    ...
    let static_count = roles.iter().filter(|r| !r.is_dynamic() && !r.is_symbolic() && r.is_parameterized()).count();
...
fn test_role_family_matching() {
    ...
    let worker_symbolic = Role::with_index(format_ident!("Worker"), RoleIndex::Symbolic("i".to_string()));
...
fn test_security_constraints() {
    ...
    assert!(matches!(over_max_static, Err(RoleValidationError::CountOverflow { .. })));
    ...
    assert!(matches!(over_max_index, Err(RoleValidationError::IndexOverflow { .. })));
    ...
    assert!(matches!(over_max_range, Err(RoleValidationError::RangeSizeOverflow { .. })));
}
...
fn test_edge_cases() {
    ...
    assert!(matches!(empty_range, Err(RoleValidationError::InvalidRange { .. })));
...
fn test_integration_dynamic_roles() {
    ...
    let has_dynamic_roles = choreography.roles.iter().any(|r| r.is_dynamic() || r.is_symbolic());
