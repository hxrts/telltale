...
use rumpsteak_aura_choreography::{
    ast::{Role, RoleParam, Choreography, Protocol, MessageType, LocalType},
    compiler::{
        parser::parse_choreography_str,
        projection::project,
        codegen::{generate_choreography_code, generate_choreography_code_with_dynamic_roles, generate_dynamic_role_support}
    }
};
...
fn bench_namespace_parsing(c: &mut Criterion) {
    ...
    group.bench_with_input(BenchmarkId::new("without_namespace", "simple"), &simple_protocol, 
        |b, protocol| {
            b.iter(|| {
                parse_choreography_str(black_box(protocol)).unwrap()
            })
        }
    );
    
    group.bench_with_input(BenchmarkId::new("with_namespace", "namespaced"), &namespaced_protocol,
        |b, protocol| {
            b.iter(|| {
                parse_choreography_str(black_box(protocol)).unwrap()
            })
...
fn bench_annotation_parsing(c: &mut Criterion) {
    ...
    for (name, protocol) in protocols {
        group.bench_with_input(BenchmarkId::new("parse", name), &protocol,
            |b, protocol| {
                b.iter(|| {
                    parse_choreography_str(black_box(protocol)).unwrap()
                })
...
fn bench_dynamic_role_projection(c: &mut Criterion) {
    ...
    group.bench_with_input(BenchmarkId::new("static_roles", "projection"), &static_choreo,
        |b, choreo| {
            b.iter(|| {
    ...
    group.bench_with_input(BenchmarkId::new("dynamic_roles", "projection"), &dynamic_choreo,
        |b, choreo| {
            b.iter(|| {
                for role in &choreo.roles {
                    let _ = project(black_box(choreo), black_box(role));
...
fn bench_code_generation(c: &mut Criterion) {
    let static_choreo = parse_choreography_str(r#"
    ...
    let dynamic_choreo = parse_choreography_str(r#"
    ...
    let static_local_types: Vec<(Role, LocalType)> = static_choreo.roles.iter()
        .map(|role| (role.clone(), LocalType::End))
        .collect();
        
    let dynamic_local_types: Vec<(Role, LocalType)> = dynamic_choreo.roles.iter()
        .map(|role| (role.clone(), LocalType::End))
        .collect();
    ...
    group.bench_with_input(BenchmarkId::new("standard", "generation"), &(&static_choreo, &static_local_types),
    ...
    group.bench_with_input(BenchmarkId::new("dynamic", "generation"), &(&dynamic_choreo, &dynamic_local_types),
        |b, (choreo, local_types)| {
            b.iter(|| {
    ...
    group.bench_function("dynamic_support_only", |b| {
        b.iter(|| {
            generate_dynamic_role_support(black_box(&dynamic_choreo))
        })
    });
...
fn bench_complex_protocol(c: &mut Criterion) {
    ...
    group.bench_function("parse_complex", |b| {
        b.iter(|| {
            parse_choreography_str(black_box(complex_protocol)).unwrap()
        })
    });
...
fn bench_scalability(c: &mut Criterion) {
    ...
    for size in [5, 10, 20, 50].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("parse", size), &protocol,
            |b, protocol| {
                b.iter(|| {
                    parse_choreography_str(black_box(protocol)).unwrap()
                })
...
fn generate_large_protocol(role_count: usize) -> String {
    let mut protocol = String::from("#[namespace = \"scalability_test\"]\nchoreography LargeProtocol {\n    roles: ");
    
    // Generate roles
    for i in 0..role_count {
        if i > 0 { protocol.push_str(", "); }
