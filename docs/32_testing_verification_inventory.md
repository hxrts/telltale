...
| Lean-backed correspondence strict suites | 4 | Curated property-suite map in `scripts/check/verification-inventory.sh` |
...
| Lean correspondence | Strict executable validation checks | `rust/bridge/tests/lean_trace_validation.rs`, `rust/bridge/tests/property_tests.rs`, `rust/bridge/tests/protocol_bundle_admission_contracts.rs`, `rust/simulator/tests/lean_reference_parity.rs` | `validateTrace`, `validateSimulationTrace`, `runSimulation`, `verifyProtocolBundle`, and deterministic reconfiguration-transition validation now execute in strict deterministic lanes. Exact full-run event-for-event Lean reference parity is still tracked separately from basic execution support |
...
| Premise | End-to-end supported/fail-closed lowering | `rust/tests/dsl_runtime_semantics_tests.rs` | Supported DSL forms now run through parser -> projection -> theory conversion -> protocol machine, including projectable authority/control-flow fixtures and parallel local-type conversion, while unsupported commitment lifecycle lowering fails closed |
...
| Full Lean protocol-machine exact reference parity | Partial | Strict lanes now execute `runSimulation` and `verifyProtocolBundle`, but exact full-run event-for-event reference parity is still narrower than basic executable support |
| Managed/discovery transport realization | Fail-closed | Loopback `Location::Remote` transport is executable, but `TopologyMode::PerRole`, `TopologyMode::Kubernetes`, and `TopologyMode::Consul` still reject explicitly until discovery-backed runtime backends exist |
